        ...
        for parameters in file_check.getParameters().values():
            for parameter in parameters:
                ...
                try:
                    ...
                    if par_id == -1:
                        raise ValueError(
                            "Parameter '" + parameter_name + "' not found in HIS-file: " + os.path.join(left_path,
                                                                                                        filename))
                    ...
                    if parameter.getLocation() is not None:
                        ...
                        if loc_id == -1:
                            raise ValueError(
                                "Location '" + parameter.getLocation() + "' not found in HIS-file: " + os.path.join(
                                    left_path, filename))
                        ...
                        if len_left != len_right:
                            raise TestCaseFailure(
                                ...
                                % (filename, param, loc, len_left, left_path, len_right, right_path))

                        diff_arr = []
                        for i in range(len_left):
                            diff_arr.append(np.abs(left_results[i, loc_id, par_id] - right_results[
                                i, loc_id, par_id]))  # sizes: diff_arr[nr_times]
                        time_id = np.argmax(diff_arr)
                        result.maxAbsDiff = float(diff_arr[time_id])

                    else:
                        diff_arr = np.abs(left_results[:, :, par_id] - right_results[:, :,
                                                                       par_id])  # sizes: diff_arr[nr_times, nr_locs]
                    ...
                    left_results[time_id, loc_id, par_id], right_results[time_id, loc_id, par_id])
                    ...
                    if result.result == "NOK":
                        try:
                            start_datetime, delta = interpret_time_unit(left_root.start_date)
                            ...
                            plot.PlotDifferencesTimeSeries(right_path, datetime_series, plot_ref_val, plot_cmp_val,
                                                           testcase_name, parameter_name, plot_location, 'his')
                        except Exception as e:
                            logging.error("Time History plot of parameter " + str(parameter.getName()) + " failed")
                            logging.exception(e)
                            local_error = True
                            result.error = True
                    ...
                    if rec.find("[General]") != -1:
                        ...
                        while True:
                            ...
                            if i == 3:
                                break
                            if j > 3:
                                raise ValueError("Block [General] does not match definition, file: " + fname)
                    if rec.find("[DioCheck]") != -1:
                        ...
                        while True:
                            ...
                            if i == 5:
                                break
                            if j > 5:
                                raise ValueError("Block [DioCheck] does not match definition, file: " + fname)
...
def interpret_time_unit(time_description):
    ...
    try:
        ...
        logging.exception(e)
        raise ValueError("Can not interpret the following unit: " + str(
            time_description) + ". A correct example is: 'T0: 2014.01.01 00:00:00  (scu=      60s)'.")
