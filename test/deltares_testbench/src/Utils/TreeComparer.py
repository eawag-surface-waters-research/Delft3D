...
# ------------------------Tree-comparison-methods------------------------ #
    ...
    def compareTreePaths(self, testbranch, refbranch, parameter, pathstr, currentresults, ignored_parameters):
        ...
        if len(notInRef) > 0:
            ...
            raise TreeException("The reference file is missing blocks: " + missingblocksstring)
        elif len(notInTest) > 0:
            missingblocksstring = ' , '.join(list(notInTest))
            raise TreeException("The test file is missing blocks: " + missingblocksstring)
        else:
            try:
                results = self.compareThisNode(testbranch, refbranch, pathstr, parameter, results, ignored_parameters)
                for key, refvals in refbranch.items():           # check from REFERENCE, key key->values
                    if key not in self.skipped_keys:
                        if key in testbranch:                    # if this key is not in the TEST ... test fails
                            ...
                            if len(testvals) != len(refvals):      # value list must have same length in test
                                raise TreeException(pathstr + '/' + key + ": Not the same size in ref and test")
                            else:
                                for testval, refval in zip(testvals, refvals):
                                    if key != 'DATA' and key != 'COLUMN INDICATION':
                                        self.compareTreePaths(testval, refval, parameter, pathstr + '>' + key, results,
                                                              ignored_parameters)
    ...
    def compare(self, ref_path, test_path, file_check, testcase_name):
        ...
        try:
            ...
            raise Exception("Cannot open reference file "+filename+' in '+ref_path)
        ...
        parameters = file_check.getParameters()['parameters']
        paramResults = []
        ...
        for parameter in parameters:
            try:
                ...
                branchpath, paramname = parameter.getName().split(':')
                ...
                if (branchpath + ':') not in ignored_parameters:
                    ...
                    try:
                        ...
                        raise Exception("Path "+branchpath+" not found in result !!")

                    # Get the branch of the reference file
                    try:
                        pad, self.refbranch = self.getBranchFromPath(self.reftree[0], branchpath)
                    except:
                        raise Exception("Path "+branchpath+" not found in reference !!")

                    # Compare the TreePaths with each other
                    newResults = self.compareTreePaths(self.testbranch, self.refbranch, parameter, pad, [],
                                                       ignored_parameters)
            ...
            for result in results:
                if result.result == 'NOK':
                    ...
                    if result.lineNumber != 0:
                        result.path = result.path + " (row: "+str(result.lineNumber)+")"
                    if result.path not in end_result.path:
                        end_result.path.append(result.path)
                        parameter.setLocation(result.path)
                if result.error:
    ...
    def pullIgnored(self, parameters):
        """
        ...
        for parameter in parameters:
            if parameter.ignore():
                ignoredparameters.append(parameter.getName())
        return ignoredparameters

    # -----------------------Node-comparison-methods----------------------- #

    def compareTableWithMissingColumn(self, resultslist, reftable, testtable, testbranch, refbranch):
        """
        ...
        if reftable.__len__() < testtable.__len__():
            ...
            columnresults.maxAbsDiffCoordinates = (testbranch['block_start'][0], testbranch['block_end'][0])
            ...
            columnresults.maxAbsDiffCoordinates = (refbranch['block_start'][0], refbranch['block_end'][0])
    ...
    def compareThisNode(self, testbranch, refbranch, pathstr, parameter, resultlist , ignored_parameters):
        """
        ...
        branchpath, varname = parameter.getName().split(':')
        local_error = False
        ...
        if (reftable is not None) and (testtable is not None):
            ...
            if reftable.__len__() == testtable.__len__():
                ...
                newresults = self.compareDataTables(reftable, testtable, pathstr, parameter)
                resultlist.extend(newresults)
            else:
                # Comparing tables where the length of the columns is not the same
                resultlist = self.compareTableWithMissingColumn(resultlist, reftable, testtable, testbranch, refbranch)
        ...
        try:
            ...
            if (pathstr + ':') not in ignored_parameters:
                if settest > set():
                    setCompareMessage += "\nKeys [%s] missing in reference." % (",".join(settest))
                if setref > set():
                    setCompareMessage += "\nKeys [%s] missing in test." % (",".join(setref))
                if setCompareMessage:  # Make sure dictionaries have the same size
                    raise TreeException("Set of variables not equal in path:\n %s.%s" % (pathstr, setCompareMessage))
                # Compare the dictionaries creates and append them to the results
                result = self.compareDictionary(testdictionary, refdictionary, pathstr, parameter)
                if result>[]:
        ...
        try:
            if ('COLUMN INDICATION' in br) and ('DATA' in br):
                ...
                for row in br['DATA'][0]['txt']:  # read data rows
                    ...
                    elif row.split('  ')[-1].strip() == 'Number of stages' or \
                            row.split('  ')[-1].strip() == 'Number of Layers':
                        pass
                    else:
                        raise TreeException("Expecting at least %d fields," % ncol +
        ...
        for key in refdictionary.keys():
            if re.search(varname, key):
                ...
                try:
                    ...
                    elif (abs(testvalue - refvalue) <= self.SetPythonCompatibility(parameter.getToleranceAbsolute()) and
                          abs((testvalue - refvalue) / refvalue) <=
                          self.SetPythonCompatibility(parameter.getToleranceRelative())):
                        result.result = "OK"
                    ...
                    # The value is not the same and is above relative Tolerances
                        ...
                        logging.info("Relative Error:   "
                                     + "test = %12.6e     ref = %12.6e (%10.2f %%): %s"
                                     % (testvalue, refvalue, result.maxRelDiff*100, result.path))
                        result.result = "NOK"
    ...
    def compareDataTables(self, reftable, testtable, pathstr, parameter):
        """
            ...
            if re.search(varname, key):
                ...
                for i in range(len(refvalue)):
                    ...
                    # values of absolute diff and relative diff within margins
                        ...
                        if isinstance(refvalue[i], str) and isinstance(testvalue[i], str):
                            logging.info("Absolute Error:   "
                                         + "test = %s     ref = %s (%s): %s(%d)"
                                         % (testvalue[i], refvalue[i], False, result.path, i))
                            result.result = "NOK"
                        elif abs(testvalue[i] - refvalue[i]) <= \
                                self.SetPythonCompatibility(parameter.getToleranceAbsolute()):
                            result.result = "OK"
                        elif abs((testvalue[i] - refvalue[i]) / refvalue[i]) <=\
                                self.SetPythonCompatibility(parameter.getToleranceRelative()):
                            result.result = "OK"
                        # absolute tolerance exceeded
                        elif abs(testvalue[i] - refvalue[i]) > self.SetPythonCompatibility(parameter.getToleranceAbsolute()):
                            result.maxAbsDiff = abs(testvalue[i] - refvalue[i])
                            result.maxAbsDiffValues = (testvalue[i], refvalue[i])
                            logging.info("Absolute Error:   "
                                         + "test = %12.6e     ref = %12.6e (%12.6e): %s(%d)"
                                         % (testvalue[i], refvalue[i], result.maxAbsDiff, result.path, i))
                            result.result = "NOK"
                        # relative tolerance exceeded
                        elif abs((testvalue[i] - refvalue[i]) / refvalue[i]) > \
                                self.SetPythonCompatibility(parameter.getToleranceRelative()):
                            result.maxRelDiff = abs((testvalue[i] - refvalue[i]) / refvalue[i])
                            result.maxRelDiffValues = (testvalue[i], refvalue[i])
                            logging.info("Relative Error:   "
                                         + "test = %12.6e     ref = %12.6e (%10.2f %%): %s%[%d%]"
                                         % (testvalue, refvalue, result.maxRelDiff*100, result.path, i))
                            result.result = "NOK"
                    results.append(result)
        return results
    ...
    def SetPythonCompatibility(self, value):
        ...
        """
        import sys
        if value == None and sys.version_info[0] >= 3:
            value = - 1
        return value
            ...
            if type(val) == dict:
                ...
                if janummer:
                    if ('block_start' in val) and ('block_end' in val):
                        linenrs = "(%d,%d)"%(val['block_start'][0],val['block_end'][0])
