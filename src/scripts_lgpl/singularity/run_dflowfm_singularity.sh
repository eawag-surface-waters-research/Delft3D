...
function print_usage_info {
    echo "Usage: ${0##*/} [run_dflowfm.sh OPTIONS]..."
    echo "       ${0##*/} [-p | --parentlevel] 3 [run_dflowfm.SH OPTIONS]..."
    echo "Runs the run_dflowfm.sh command of a Singularity container by wrapping and passing additional arguments."
    echo
    echo "Options:"
    echo "-h, --help"
...
singularity exec --cleanenv --bind $working_dir:$mountdir --pwd $container_working_dir --env PATH=$restricted_path $container_file_path $container_libdir/$executable $executable_extraopts
