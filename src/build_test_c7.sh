...
case $compiler in
    ...
    intel16)
        echo "We are not using Intel 16.0.3 Fortran ($platform) compiler any more"
		exit 1
        ;;

    intel14)
        echo "We are not using Intel 14.0.3 Fortran ($platform) compiler any more"
		exit 1
        ;;
    intel12)
        echo "We are not using Intel 12 Fortran ($platform) compiler any more"
		exit 1
        ;;

    intel11.1)
        echo "We are not using Intel 11.1 Fortran ($platform) compiler any more"
		exit 1
        ;;

    intel11.0)
        echo "We are not using Intel 11.0 Fortran ($platform) compiler any more"
		exit 1
        ;;

    intel10)
        echo "We are not using Intel 10.0 Fortran ($platform) compiler any more"
		exit 1
        ;;
...
log='logs/make_utils_gpl.log'
command="FC=mpif90 make ds-install -C utils_gpl &> $log"
log "Running $command"
eval $command
if [ $? -ne 0 ]; then
    log "ERROR: Make utils_gpl fails!"
    cd $orgdir
    exit 1
fi
...
#
# Tests
log='logs/make_tests.log'
command="FC=mpif90 make ds-install -C test &> $log"
log "Running $command"
eval $command
if [ $? -ne 0 ]; then
    log "ERROR: Make tests fails!"
    cd $orgdir
    exit 1
fi
...
command="/opt/apps/anaconda3/2020.02/bin/python $maindir/engines_gpl/dimr/scripts/dimr_artifacts.py $maindir &> $log"
