...
! is caused by the fact that n1dedges=0, but still we have a 1D2D mesh contact to be written later.
   ...
   if (ndx2d > 0) then ! 2D flow geometry
      ...
      !Write bottom level and water level for sigma coordinates and Fourier specific versions as required.
      if(layer_type == LAYERTYPE_OCEAN_SIGMA_Z .or. layer_type == LAYERTYPE_OCEAN_SIGMA) then
          ...
          if ( present(jafou)) then
              ierr = nf90_def_var(ncid, trim(mesh2dname)//'_wlev' , nf90_double, id_tsp%meshids2d%dimids(mdim_face), id_tsp%meshids2d%varids(mid_wlev))
              ierr = nf90_put_att(ncid, id_tsp%meshids2d%varids(mid_bldepth), 'standard_name' , "water level on cell centres" )
              ierr = nf90_put_att(ncid, id_tsp%meshids2d%varids(mid_bldepth), 'units', "m" )
              ierr = nf90_put_att(ncid, id_tsp%meshids2d%varids(mid_bldepth), 'positive',  "down" )
              
              ierr = nf90_enddef(ncid)
              ierr = nf90_put_var(ncid, id_tsp%meshids2d%varids(mid_wlev), s1Fourier(1:ndx2d))
              ierr = nf90_put_var(ncid, id_tsp%meshids2d%varids(mid_bldepth), -1*blFourier(1:ndx2d))
              waterlevelname = '_wlev'
          else
              ierr = nf90_enddef(ncid)
