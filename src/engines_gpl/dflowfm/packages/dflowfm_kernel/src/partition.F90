...
!        make CSR arrays zero-based
      ...
      case (0,1)
         ...
         if (ierror /= METIS_OK .and. jacontiguous == 1) then
            call mess(LEVEL_WARN, 'The above METIS error message is not a problem. It means that partitioning failed for k-way method with option contiguous=1 because the input graph is not contiguous. Retrying partitioning now with contiguous=0.')
