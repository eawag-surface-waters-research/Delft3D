...
!!--declarations----------------------------------------------------------------
    ...
    use m_flowgeom, only: ndxi, bl, kfs, lnxi, lnx, ln, dxi, ndx, csu, snu, wcx1, wcx2, wcy1, wcy2, acl, nd, csu, snu, wcl, xz, yz, xu, yu
    use m_flow, only: s0, s1, u1, u0, v, ucx, ucy, kbot, ktop, kmx, kmxn, plotlin, sa1, tem1, zws, hs, ucxq, ucyq, layertype, &
                      iturbulencemodel, z0urou, frcu, ifrcutp, hu, spirint, spiratx, spiraty, u_to_umain
...
!
    ...
    !
    ! Allocate memory
    allocate(dzdx(1:ndx), dzdy(1:ndx), stat=ierr)
    allocate(localpar (stmpar%trapar%npar), stat = ierr)
    allocate(ua(1:ndx), va(1:ndx), stat=ierr)
    allocate(z0rouk(1:ndx), z0curk(1:ndx), taub(1:ndx), deltas(1:ndx), stat=ierr)
...
!             ! 3D CASE
   ...
   deallocate(dzdx, dzdy, u1ori, u0ori, vori, stat = ierr)
