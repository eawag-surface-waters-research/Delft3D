    ...
    if (jased > 0 .and. stm_included) then
      ...
      if (ndx1d > 0 .and. stm_included) then
         if (stmpar%morpar%bedupd) then
            ...
            ierr = nf90_def_var(irstfile, trim(mesh1dname)//'_mor_crs_n', nf90_double, (/ id_jmax, id_ncrs, id_timedim /), id_flowelemcrsn)
            ierr = nf90_put_att(irstfile, id_flowelemcrsn, 'long_name','cross-section points width')
...
!           3D
    ...
    if (ndx1d > 0 .and. stm_included) then
       if (stmpar%morpar%bedupd) then
          ...
          ierr = nf90_put_var(irstfile, id_flowelemcrsn, work1d_n(1:jmax,1:nCrs), start=(/ 1, 1, itim /), count=(/ jmax, nCrs, 1 /) )
...
!   type(t_crs) :: pj
   ...
   if (ndx1d > 0) then
      ...
      !         jmax = max(jmax,pCSs(i)%levelscount)
      !      enddo
      !      ierr = nf90_def_dim(ncid, trim(mesh1dname)//'_crs_maxdim', jmax, id_tsp%id_jmax)
      !      ierr = nf90_def_dim(ncid, trim(mesh1dname)//'_ncrs'      , nCrs, id_tsp%id_ncrs)
      !      ierr = nf90_def_var(ncid, 'flowelem_crs_z', nf90_double, (/ id_tsp%id_jmax, id_tsp%id_ncrs /), id_tsp%id_flowelemcrsz(1))
      !      if (jased > 0) then
...
!   ierr = unc_def_var_map(mapids, mapids%id_flowelemcontourx(:), nf90_double, UNC_LOC_S, 'FlowElemContour_x', '', '', 'm', (/ id_flowelemcontourptsdim, id_seddim, -1 /).
   ...
   !         enddo
   !      enddo
   !      ierr = nf90_put_var(ncid, id_tsp%id_flowelemcrsz(1), work1d_z(1:jmax,1:nCrs), start=(/ 1, 1 /), count=(/ jmax, nCrs /) )
   !      ierr = nf90_put_var(ncid, id_tsp%id_flowelemcrsn(1), work1d_n(1:jmax,1:nCrs), start=(/ 1, 1 /), count=(/ jmax, nCrs /) )
   !      deallocate (work1d_z, work1d_n)
   !   endif
   !endif
