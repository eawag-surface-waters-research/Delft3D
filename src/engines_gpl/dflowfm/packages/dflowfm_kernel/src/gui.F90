...
! $Id: gui.F90 52266 2017-09-02 11:24:11Z klecz_ml $
! $HeadURL: https://repos.deltares.nl/repos/ds/branches/dflowfm/20161017_dflowfm_codecleanup/engines_gpl/dflowfm/packages/dflowfm_kernel/src/gui.F90 $
! Copyright notice:
...
!         call findcells(0)          ! do not include folded cells
         call delete_drypoints_from_netgeom(dryptsfile,0)
      ...
      ELSE
         ...
         ELSE
            ...
            if (nwhat.eq.21) then
               ...
               call unc_write_net(filnam, janetcell = 1, janetbnd = 1)
...
!      ENDIF
   ...
   ELSE IF (NWHAT .EQ. 37 ) THEN    ! partition files
      if ( npartition_pol.lt.1 .and. md_genpolygon == 1 ) then
...
!  vectors
   ...
   integer, parameter :: MAXOP = 128   ! 54
...
1234 continue
   ...
   ELSE IF (NWHAT .EQ. 10) THEN ! flow nodes
      ...
      if( jasecf > 0 ) then
         ...
         OPTION(46)= 'ground water pressure                (m)'
      else if (jawave > 0) then 
         OPTION(46)= 'Significant wave height              (m)'
      else
         OPTION(46)= '                                        '
      endif
      ...
      else if (nonlin == 2) then
         OPTION(47)= 'a1m                                 (m2)'
      else if (jawave > 0) then 
         OPTION(47)= 'Significant wave period              (s)'
      else
         OPTION(47)= '                                        '
      endif
      ...
      if( jasecf > 0 ) then
         ...
         OPTION(48)= 'Wave Orbital velocity uorb         (m/s)'
      else
         OPTION(48)= '                                        '
      endif
...
!                    CALL IGRJOIN(real(x(1)),real(y(1)),real(x(2)),real(y(2)))
  ...
  ELSE IF (NCOL == 1) THEN ! 1 MEANS: DRAW IN KN3 PREDEFINED COLOUR
     if (KN(3,L) == 0) then
        ...
        NCL = NCOLRN 
     else IF (KN(3,L) == 5) THEN ! 1d2d internal pipe
        NCL = NCOLSP 
     ENDIF
  ELSE
     NCL = NCOL
...
!
   ...
   OPTION(10) = 'Slotw2D                              (m)' ; it(2*10) = 6
   OPTION(11) = 'Epsmaxlev                            (m)' ; it(2*11) = 6
   OPTION(12) = 'Epsmaxlevm                           (m)' ; it(2*12) = 6
   OPTION(13) = 'jawavestreaming terms in D3Dwavemodel( )' ; it(2*13) = 2
   OPTION(14) = 'jawaveStokes 0,1,2                   ( )' ; it(2*14) = 2
   OPTION(15) = 'jawaveRoller                         ( )' ; it(2*15) = 2
   OPTION(16) = 'Maxitforestersal                     ( )' ; it(2*16) = 2
   OPTION(17) = 'Maxitforestertem                     ( )' ; it(2*17) = 2
   OPTION(18) = 'Jajipjan (Noderivedtypes in mdu)     ( )' ; it(2*18) = 2
   OPTION(19) = 'Maxdegree                            ( )' ; it(2*19) = 2
   OPTION(20) = 'Jaevap                               ( )' ; it(2*20) = 2
...
!            1         2         3         4         5         6
   ...
   HELPM (10) = 'Slotwidth in 2D , default 1d-3                          (m) '
   HELPM (11) = 'Max level diff in Newton iterations,      default 1d-8  (m) '
   HELPM (12) = 'Max level diff in outer loop of Nested Newton def 1d-8  (m) '
   HELPM (13) = '>=1 streaming, >= 2 streaming + turb                        '
   HELPM (14) = '0=no, 1 = uniform, 2 = non-uniform, 3=2+vertical visc Stokes'
   HELPM (15) = '0=no, 1 = rol1, 2 = rol2                                    '
   HELPM (16) = 'Max nr of iterations                                        '
   HELPM (17) = 'Max nr of iterations                                        '
   HELPM (18) = '0=use der. types, 1 = less, 2 = lesser, 5 = also deallo der.'
   HELPM (19) = '6 = default, 666 = number of the devil                      '
   HELPM (20) = '1 = evaporation computed bij heatfluxmodel , 0= no evap     '
...
!  for 2 of the 4 input fields
   ...
   CALL IFORMputdouble  (2*10 ,Slotw2D          , '(E8.2)' )       
   CALL IFORMputdouble  (2*11 ,Epsmaxlev        , '(E8.2)' )       
   CALL IFORMputdouble  (2*12 ,Epsmaxlevm       , '(E8.2)' )     
   CALL IFORMputinteger (2*13 ,jawavestreaming             )     
   CALL IFORMputinteger (2*14 ,jawaveStokes                )     
   CALL IFORMputinteger (2*15 ,jawaveRoller                )     
   CALL IFORMputinteger (2*16 ,Maxitverticalforestersal    )     
   CALL IFORMputinteger (2*17 ,Maxitverticalforestertem    )     
   CALL IFORMputinteger (2*18 ,Jajipjan                    )     
...
!  check for Help, Confirm, Quit
   ...
   ELSE IF (KEY .EQ. 22 .OR. KEY .EQ. 23) THEN
       IF (KEY .EQ. 22) THEN
          ...
          CALL IFORMGETdouble  (2*10 ,Slotw2D          )
          CALL IFORMGETdouble  (2*11 ,Epsmaxlev        )
          CALL IFORMGETdouble  (2*12 ,Epsmaxlevm       )
          CALL IFORMGETinteger (2*13 ,jawavestreaming         )  
          CALL IFORMGETinteger (2*14 ,jawaveStokes            )  
          CALL IFORMGETinteger (2*15 ,jawaveRoller            )  
          CALL IFORMGETinteger (2*16 ,Maxitverticalforestersal)     
          CALL IFORMGETinteger (2*17 ,Maxitverticalforestertem)     
          CALL IFORMGETinteger (2*18 ,Jajipjan                )     
   ...
   integer :: numpar, numfld, numparactual, numfldactual
   PARAMETER  (NUMPAR = 1, NUMFLD = 2*NUMPAR)
   INTEGER  IX(NUMFLD),IY(NUMFLD),IS(NUMFLD),IT(NUMFLD)
   CHARACTER WRDKEY*40, OPTION(NUMPAR)*40, HELPM(NUMPAR)*60
   integer :: nlevel
   COMMON /HELPNOW/ WRDKEY,NLEVEL
...
!  for 2 of the 4 input fields
   ...
   CALL IFormPutDouble  (2*12 ,Tlfsmo           ,'(F10.2)')
   CALL IFormPutDouble  (2*13 ,Dtfacmax         ,'(F10.2)')
...
!
   ...
   OPTION(14) = 'uniform rainfall              (mm/hr)   ' ; it(2*14) = 6        
...
!            1         2         3         4         5         6
   ...
   HELPM (14) = '(if non-zero overrides ext forcings)                        '
...
!  for 2 of the 4 input fields
   ...
   CALL IFormPutDouble  (2*14 , rainuni   , '(F8.3)')
...
!  check for Help, Confirm, Quit
   ...
   ELSE IF (KEY .EQ. 22 .OR. KEY .EQ. 23) THEN
       IF (KEY .EQ. 22) THEN
           ...
           CALL IFormGetDouble  (2*14 , rainuni)
...
!     check for Help, Confirm, Quit
   ...
   implicit none
   integer :: numpar, numfld, numparactual, numfldactual
   PARAMETER  (NUMPAR = 7, NUMFLD = 2*NUMPAR)
   INTEGER  IX(NUMFLD),IY(NUMFLD),IS(NUMFLD),IT(NUMFLD)
...
!                     hw    = 0.25d0*( a1(k1) + a1(k2) )/dx(L)
 ...
 if (kmx == 0) then
    ...
    if (nonlin == 2) then 
       call tekrailines(ncolana,ymn,1,6) ! s1
    endif   
 endif
...
! make string for local time-stepping
 ...
 if (kmx == 0) then 
    ...
    ' #s1it: ', nums1it, ' iad: ', iadvec, limtypmom, limtypsa, javasal, javau,  ' runid: '//trim(md_ident), ' ', c_lts
 else
    call getlink1(nn,LL) 
 ...
 ! call TEKFN(3, 6, 1, qw(kb:kt)    , hwref(1:) , km, vmin, vmax, zmin, zmax, KLAXS, 'qw'    , 1, 2 , 0d0,kplot)
  ...
  if (jatem > 0) then 
     if (jafahrenheit > 0) then 
        dijdij(1:km) = 32d0 + tem1(kb:kt)*1.8d0
        call getvminmax(7,vmin,vmax,dijdij, km) 
        vmin = 70d0 ; vmax = 90d0
        call TEKFN(7, 13, 1, dijdij(1:km) , hcref   , km, vmin, vmax, zmin, zmax, KLPROF, 'TFahrenheit' , 0, 2 , 0d0,kplot)
     else   
       call getvminmax(7,vmin,vmax,tem1(kb:), km) 
       call TEKFN(7, 13, 1, tem1(kb:kt) , hcref   , km, vmin, vmax, zmin, zmax, KLPROF, 'temperature' , 0, 2 , 0d0,kplot)
     endif   
...
do n = 1,numsrc ! teksorsin
   ...
   if (k .ne. 0) then 
      ...
      if ( inview(xp,yp) ) then 
          ...
          else if (ndraw(41) == 5) then 
             if (qsrc(n) < 0d0) then 
                write(tex,'(f10.3)') sasrc(n)
                call gtext(trim(tex)//' (ppt)', xp, yp , klsrc)
             endif   
          else if (ndraw(41) == 6) then 
             if (qsrc(n) < 0d0) then 
                write(tex,'(f10.3)') tmsrc(n)
...
subroutine editGui(MODE, NFLD,KEY,lastmode)
    use network_data, only: netflow
    implicit none
    integer :: MODE,NFLD, KEY, lastmode
