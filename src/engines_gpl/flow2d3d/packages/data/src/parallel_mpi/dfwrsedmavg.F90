subroutine dfwrsedmavg(lundia    ,error     ,trifil    ,nst       ,mmax      , &
...
!    
    ...
    real(fp),    dimension(:,:,:,:), allocatable  :: rsbuff
!
! Data statements
!
...
!    
 ...
 1010  continue
    ...
    !
    !i = 0
    allocate( rsbuff(gdp%d%nlb:gdp%d%nub, gdp%d%mlb:gdp%d%mub, lsedtot, 1) )
    do l = 1, lsedtot
       select case(moroutput%transptype)
       case (0)
          rhol = 1.0_fp
       ...
       do m = 1, mmax
          do n = 1, nmaxus
             ...
             if ( dmorft > 0.0_hp ) then
                ...
                rsbuff(n,m,l,1) = real(sbuuc(nm, l)/rhol/(real(dmorft*86400.0_hp,fp)),fp)
             else
                !sbuff(i) =  0.0_sp
       ...
       do m = 1, mmax
          do n = 1, nmaxus
             ...
             if ( dmorft > 0.0_hp ) then
                ...
                rsbuff(n,m,l,1) = real(sbvvc(nm, l)/rhol/(real(dmorft*86400.0_hp,fp)),fp)
             else
                !sbuff(i) =  0.0_sp
                rsbuff(n,m,l,1) =  0.0_fp
             endif
