...
!
    ...
    if (filgrd /= fildef) then
       ...
       !
       ! Initialize "global" array MNGRD (2,MXNPPT)
       !
       do n = 1, mxnppt
          mngrd(1, n) = 0
          mngrd(2, n) = 0
  ...
  110  continue
       ...
       if (lerror) goto 200
       !
       ! define mngrd, and test values
       !
       mngrd(1, ippt) = ival(1)
       mngrd(2, ippt) = ival(2)
       if (mngrd(1, ippt)==0 .or. mngrd(2, ippt)==0) then
          if (imng == 1) then
             !
          ...
          !
          ! write mngrd to semi-scratch file if noui = .true.
          !
          if (noui) then
             write (lungrd) (mngrd(j, ippt), j = 1, 2)
          endif
          !
          ! check if ippt exceeds maximum value, then ippt will be
          ! reset, for noui = .true. this will never occur
          ! reset 1 to large, because ncpgrd = ippt - 1
          !
          if (.not.noui) ippt = ippt + 1
          imng = imng + 1
          !
          if (ippt > mxnppt) then
             lerror = .true.
       ...
       !
       ! stop reading define flgrd
       !
       !
       ! define actual number of comp. grid enclosure points
       ! and define flgrd
       !
  200  continue
       ncpgrd = ippt - 1
