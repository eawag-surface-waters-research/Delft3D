...
!
    ...
    real(fp), dimension(*)          :: buffr  !!  User supplied buffer to read from or
...
!
  ...
  100 continue
    !
    ! write to and read from NEFIS in single precision
    !
    ibuflen = buflen/nbytsg(j)
    call sbuff_checksize(ibuflen)
    if (wrilog) then
       sbuff(1:ibuflen) = real(buffr(1:ibuflen), sp)
       error = putelt(fd_nef, grpnam, elmnam, uindex, 1, sbuff)
    else
       error = getelt(fd_nef, grpnam, elmnam, uindex, 1, buflen, sbuff)
       if (error/=0) goto 9999
       buffr(1:ibuflen) = real(sbuff(1:ibuflen), fp)
    endif
    !
    ! error:
