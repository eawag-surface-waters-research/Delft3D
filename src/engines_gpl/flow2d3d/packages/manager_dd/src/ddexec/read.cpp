...
DD_ReadDD (
    ...
    Log (LOG_DETAIL, "Reading Delft3D (XML) config file \"%s\"", filename);
    ...
    Log (LOG_DETAIL, "Done reading Delft3D (XML) config file \"%s\"", filename);
    return state->nodes;
    }
...
starttag (
    ...
    if (strcmp (name, "cluster") == 0) {
        ...
        if (state->nodes > MAXCLUSTERS)
            Abort ("Too many nodes (%d) for max clusters (%d)", state->nodes, MAXCLUSTERS);

        Log (LOG_DETAIL, "Creating %d clusters", state->nodes);
