...
///--description-----------------------------------------------------------------
//
// Class: D3dFlowMapper, configuration part
//
// Configuration object for DELFT3D-FLOW Domain Decomposition
//
    ...
    )
{
    ...
    if (numRead != 10)
        Abort ("Cannot parse configString \"%s\" (numRead = %d)", configString, numRead);
    else
    {
        if ( mStartLeft == mEndLeft )
        {
            ...
            {
                Abort ("Inconstistent Mapper direction configString \"%s\"", configString);
            }
        }
        else if ( nStartLeft == nEndLeft )
        {
            ...
            {
                Abort ("Inconstistent napper direction configString \"%s\"", configString);
            }
        }
        else
        {
            Abort ("M1/M2 or N1/N2 must be equal in configString \"%s\"", configString);
        }
    }
    ...
    switch ( edgeType[C_0] )
    {
        ...
        default:
            ...
            Abort ("Error: Determined wrong edgeType from configString \"%s\"", configString);
            break;
    }
	...
	if ( echoRefinement == 1) {
        if ((refinementFile = fopen (refFileName, "a")) == NULL) {
            Abort ("Cannot open refinement file \"%s\" for writing.", refFileName);
        }
	}
    ...
    for ( int ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        {
            Abort ("Error in determining refinements, configString\"%s\"", configString);
        }
    ...
    )
{
    ...
    if ( retVal != HY_OK )
    {
        Abort ("Call to configStringParser failed for configString \"%s\"", configString);
    }
...
int D3dFlowMapper::CheckConfig()
{
    ...
    if (    check[C_0] == HY_ERR
         ...
         || checkBoth  == HY_ERR  )
    {
        Abort ("Errors found in config file %s", this->confFile);
        retVal = HY_ERR;
    }
    ...
    )
{
    ...
    case Vel_Tang:
            ...
            break;
    default:
            Abort ("Unexpected case (%d) in GetStartCell", orient);
            break;
    }
#if LOG_CELLS
    Log (LOG_ITER_MAJOR, "StartCell for eq %d: %3d", eq, startCell);
#endif
    return startCell;
}
    ...
    )
{
    ...
    case Vel_Tang:
            ...
            break;
    default:
            Abort ("Unexpected case (%d) in GetEndCell", orient);
            break;
    }
#if LOG_CELLS
    Log (LOG_ITER_MAJOR, "EndCell for eq %d: %3d", eq, endCell);
#endif
    return endCell;
}
    ...
    )
{
    ...
    {
        Abort ("Invalid NormalCell in GetNormalCell");
    }
    ...
    )
{
    ...
    {
        retVal = HY_ERR;
        Warn ("Edge type not available in config file (ctx %d)", aCtx);
    }
    ...
    {
        retVal = HY_ERR;
        Warn ("Couldn't get First or Last cell from config file (ctx %d)", aCtx);
    }
    ...
    Log (LOG_ITER_MAJOR, "Checking configuration for Context %d", aCtx );

    if (   (Edge[aCtx] < 0) || (Edge[aCtx] >= NR_EDGETYPES) )
    {
        retVal = HY_ERR;
        Warn ("Couldn't get Edge type  from config file (ctx %d)", aCtx);
    }

    if ( FirstCell[aCtx] >= LastCell[aCtx] )
    {
        retVal = HY_ERR;
        Warn ("FirstCell must be < LastCell");
    }

    if ( NormalCell[aCtx] == YET_TO_INIT )
    {
        retVal = HY_ERR;
        Warn ("NormalCell must be Specified");
    }

    if ( FirstCell[aCtx] < 1 )
    {
        Warn ("First Cell (%d) < 1", FirstCell[aCtx]);
        retVal = HY_ERR;
    }

    if ( LastCell[aCtx] > domMaxCell )
    {
        Warn ("Last Cell (%d) for \"%s\"-\"%s\" exceeds maximum (%d)", LastCell[aCtx],
...
int D3dFlowMapper::CheckConfigBothContexts(void)
{
    ...
    Log (LOG_ITER_MAJOR, "Checking configuration between Contexts");

    if ( (Ref[C_0] != 1) && (Ref[C_1] != 1) )
    {
        Warn ("One of the Refinement factors must be 1");
        retVal = HY_ERR;
    }
