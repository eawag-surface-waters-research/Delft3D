...
#include "flow_nxtstp.h" // for PrintNextStepName
    ...
    )
{
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        {
            Abort("D3dFlowMapper::Setup: # incoming messages must be zero");
        }
    ...
    Log(LOG_ITER_MAJOR, "MAPPER \"%s\" Setup, retVal %d, Checking config", IteratorSelf()->Name(), retVal);

    retVal = this->CheckConfig();

    Log(LOG_ITER_MAJOR, "MAPPER \"%s\" Setup, Checked config, retVal %d", IteratorSelf()->Name(), retVal);
    ...
    )
{
    ...
    if ( myStep != STEP_UNDEF )
    {
        for ( int ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
        {
            ...
            Log(LOG_ITER_MAJOR, "MAPPER NEXTSTEP to Flow \"%s\": myStep %s, neighborStep %s, numOut %d",
    ...
    if ( myStep != STEP_UNDEF ) // TODO Select group on myStep
    {
        for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
        {
            ...
            if ( updateHeader.numMessages < 0 )
            {
                Abort("D3dFlowMapper::NextStep: # incoming messages must >= 0");
            }
    ...
    if ( nextStep != myNxtStep )
    {
        ...
        {
#if 0       // TODORE: CHECK
            Abort ("Unexpected step (%d) after step %d; UNEXPECT %s after %s",
                            nextStep,
                            myStep,
    ...
    if ( totNumIn > 0 )
    {
        ...
        {
            Abort("D3dFlowMapper::NextStep: #inmessages inconsistent with #flow processes");
        }
        else
        {
