...
//
// Parse mapper config string (TODORE: in separate include file)
//

extern int ParseMapperConfigString(
    ...
    )
{
    ...
    Log(LOG_ITER_MAJOR, "GAWS \"%s\" SETUP", gaws->Name());

    this->InitializeMapperInfo();

    {for ( int m = 0 ; ( m < this->numMappers ) && ( retVal == HY_OK ) ; m++ )
    {
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" fills mapper cells for mapper %d", gaws->Name(), m);
    ...
    if ( retVal != HY_ERR )
    {
        ...
        {
            Abort("CreateWangAdministration(Wang_YDir) UNSUCCESFULL");
        }
    }
    else
    {
    ...
    {for ( int p = 0 ; ( p < this->nProc ) && ( retVal == HY_OK ) ; p++ )
    {
        Log(LOG_ITER_MAJOR, "GAWS Sets up communication for process %d", p);
...
void Gaws::DoSolving(void)
{
    ...
    while (! finished)
    {
        ...
        {
            Log(LOG_ITER_MAJOR, "GAWS \"%s\" WAITS for L2R message from flow \"%s\"",
                            IteratorSelf()->Name(), this->contexts[p]->flowIterator->Name());
            ...
            {
                Abort("Gaws::Setup: DoSolving blobID (%d /= %d)",
                            incomingBlobID, F2G_Blob_Left2RightMessage);
            }
            Log(LOG_ITER_MAJOR, "GAWS \"%s\" RECEIVED L2R message (%d) from flow \"%s\"",
                            IteratorSelf()->Name(), inMesg[p].intValue, this->contexts[p]->flowIterator->Name());
        ...
        {for ( int p = 0 ; p < this->nProc ;  p++ )
        {
            if ( inMesg[p].intValue != inMesg[0].intValue )
                Abort("GAWS barrier: flow processes out of L2R/T2B phase");
        }}
        ...
        else
        {
            ...
            {for ( int p = 0 ; p < this->nProc ;  p++ )
            {
                ...
                Log(LOG_ITER_MAJOR, "GAWS \"%s\" SENDS OK message to flow \"%s\"",
                                IteratorSelf()->Name(), this->contexts[p]->flowIterator->Name());

                this->contexts[p]->flowIterator->Send(okMesgBlob, G2F_Blob_OKMessage); // DDD-COMM: GAWS->FLOW, ok message

                Log(LOG_ITER_MAJOR, "GAWS \"%s\" HAS SENT OK message to flow \"%s\"",
                                IteratorSelf()->Name(), this->contexts[p]->flowIterator->Name());
    ...
    )
{
    ...
    {
        Abort ("Call to configStringParser failed for configString \"%s\"", mapper.configStr);
    }
    else
    {
        for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
        {
            if (    firstCell[ctx] == YET_TO_INIT
                 ...
                 || normalCell[ctx]== 0           )
            {
                Abort ("Could not read required info from  configString \"%s\"", mapper.configStr);
            }
        }
        ...
        switch ( edgeType[C_0] )
        {
            ...
            Abort ("Wrong Edge Type in FillMapperCells");
            break;
    ...
    )
{
    ...
    if (kcs != 3)
    {
        Abort ("Inconsistent Coupling indices");
    }
    else
    {
        curCell++;
    ...
    )
{
    ...
    if ( DoLoggingToFileFor(Log_Gaws) )
    {
        if (logFile == NULL)
        {
            ...
            if (logFile == NULL)
            {
                Abort ("Couldn't open logfile %s", name);
            }
        }
        if ( logFile != NULL )
        {
    ...
    )
{
    ...
    if (retVal == -999)
    {
        Abort ("ERROR returned in GAWS::CheckDependents.\n");
    }
    ...
    )
{
    if ( nSeries % EQ_SER_BLOCK_SIZE == 0)
    {
        ...
        if ( eqSeries == NULL )
        {
            Abort("NewEquationSerie: could not realloc %d series", nNewSeries);
        }
    }
    //
    ...
    )
{
    ...
    if ( eqSeries[eqSerie].nPnt % EQ_PNT_BLOCK_SIZE == 0)
    {
        ...
        {
            Abort("AddEqPoint: could not realloc %d points", nNewPoints);
        }
    }
    ...
    )
{
	...
	if ( success == 0 )
	{
        Abort ("Aborting due to errors in GAWS::SetDependentLevels.\n");
	}
}
    ...
    )
{
    ...
    if ( retVal != HY_OK )
    {
        Abort ("ERROR returned in GAWS::ProcessCouplePoint on calling FindNeighborPoint.\n");
    }
    ...
    if ( numDep >= 0 )
    {
        ...
        if ( retVal != HY_OK )
        {
            Abort ("ERROR returned in GAWS::ProcessCouplePoint on calling AddEqPoint.\n");
        }
    }
    else
    {
...
void Gaws::InitializeMapperInfo(void)
{
    ...
    Log(LOG_ITER_MAJOR, "GAWS \"%s\" intializes mapper info", this->gawsIterator->Name());

    Category * d3dFlowProcs = LookupCategory ((char *) DDProcessCategory);
    Category * mappers      = LookupCategory ((char *) DDMapperCategory);

    if (mappers == NULL)
        Abort ("\"mapper\" category does not exist (GAWS: \"%s\")", this->gawsIterator->Name());

    if (d3dFlowProcs == NULL)
        Abort ("\"process\" category does not exist for (GAWS: \"%s\")", this->gawsIterator->Name());

    int numD3dFlowProcs = this->gawsIterator->NeighborCount (d3dFlowProcs);

    if (numD3dFlowProcs >= MAX_NUM_D3D_FLOW_PROCS)
        Abort ("GAWS iterator \"%s\" has too many d3dflow processes", this->gawsIterator->Name());
    ...
    for ( int p = 0 ; p < numD3dFlowProcs ; p++ )
    {
        ...
        {for ( int m = 0 ; m < numFlowMappers; m++ )
        {
            ...
            {for ( int map = 0 ; ( map < this->numMappers ) && (! allReadyInMapInfo ) ; map++ )
                ...
                {
                    Log(LOG_ITER_MAJOR, "GAWS \"%s\": mapper \"%s\" already added", this->gawsIterator->Name(), mapper->Name());
            ...
            if ( ! allReadyInMapInfo )
            {
                ...
                {
                    Abort ("GAWS iterator \"%s\" found too many mappers", this->gawsIterator->Name());
                }
                else
                {
                    ...
                    Log(LOG_ITER_MAJOR, "GAWS: mapper \"%s\", configString \"%s\"",
                    ...
                    Log(LOG_ITER_MAJOR, "GAWS \"%s\" added mapper \"%s\" (flow \"%s\" -> \"%s\"), configString \"%s\"",
    ...
    if (this->numMappers < 1)
        Abort ("GAWS iterator \"%s\" did not find any mappers", this->gawsIterator->Name());
    else
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" found %d mappers", this->gawsIterator->Name(), this->numMappers);
}
