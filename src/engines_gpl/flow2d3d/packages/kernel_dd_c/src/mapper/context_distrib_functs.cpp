...
//
// Context Attacher, base class (parent for Flow side
//                               Map side, Gaws Side)
//
...
void D3dFlowContext::DetermineStripSize(void)
{
    ...
    for (eq = 1; eq < NR_EQ; eq++)
    {
        ...
        // printf("QQQ-1 %2d (%2d %2d) (%2d %2d)\n",eq,mStartMin,mEndMax,nStartMin,nEndMax);
    }
    ...
    {
#if 1
        case Edge_Left:    mEndMax++;
            break;
        case Edge_Right:   mStartMin--;
            break;
        case Edge_Bottom:  nEndMax++;
            break;
        case Edge_Top:     nStartMin--;
            break;
#else
        case Edge_Left:                 mEndMax++; nStartMin--; nEndMax++;
            break;
        case Edge_Right:   mStartMin--;            nStartMin--; nEndMax++;
            break;
        case Edge_Bottom:  mStartMin--; mEndMax++;              nEndMax++;
            break;
        case Edge_Top:     mStartMin--; mEndMax++; nStartMin--;
            break;
#endif
    }
...
int D3dFlowContext::FillBlockAdmin(void)
{
    ...
    Log(LOG_ITER_MAJOR, "FlowConfig \"%s\" for Mapper \"%s\": mAO %4d, nAO %4d, mAS %4d, nAS %4d",
                    this->flowIterator->Name(), // ConfigBlob()->Address(),
                    this->mapperIterator->Name(),
                    mArrayOffset, nArrayOffset, mArraySize, nArraySize);
    Log(LOG_ITER_MAJOR, "FlowConfig \"%s\" for Mapper \"%s\": mStartMin %4d, mEndMax %4d, mStripSize %4d, nStripSize %4d, OFF %4d",
                    this->flowIterator->Name(), // ConfigBlob()->Address(),
                    this->mapperIterator->Name(),
