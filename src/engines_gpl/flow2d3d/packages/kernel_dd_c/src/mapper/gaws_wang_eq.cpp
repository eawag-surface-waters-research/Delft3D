    ...
    )
{
    ...
    if ( ! BeginOfLine )
    {
        ...
        ON_LOG_EQUATIONS(
            ...
            Log(LOG_ITER_MAJOR, "ADD-EQ-END(%3d): %6d(%3d,%3d) %10.4f %10.4f %10.4f %10.4f",
                numEq,
                cntxt->GetContextId(),
                curM, curN,
...

    {
        ...
        ON_LOG_EQUATIONS(
            ...
            Log(LOG_ITER_MAJOR, "ADD-EQ-BEGIN(%3d): %6d(%3d,%3d) %10.4f %10.4f %10.4f %10.4f",
                numEqP,
                cntxt->GetContextId(),
                curM, curN,
...
#if LOG_SOLVE
        // Log(LOG_ITER_MAJOR, "solution: (%6d)) %10.5e %10.5e %10.5e %10.5e", n,
        printf("solution: (%6d)) %10.5e %10.5e %10.5e %10.5e\n", n,
                *A,*B,*C,*D );
#endif
    }
...
#if LOG_SOLVE
            // Log(LOG_ITER_MAJOR, "solution other domain: (%6d) %10.5e", n,*H );
            printf("solution other domain: (%6d) %10.5e\n", n,*H );
#endif
        }
    }
    ...
    for ( n = 0 ; n < this->numEq ; n++ )
    {
        if ( ! this->equations[n].endOfRow )
        {
            ...
            *Hprev = *D;
            ...
            // DxDyLast,DxDyFirst );
            // Log(LOG_ITER_MAJOR, "solution couple pt. left dom.: %10.5e %10.5e",*D,*H );
            printf("solution couple pt. left dom.: %10.5e %10.5e\n",*D,*H );
#endif
        }
    }
    ...
    for ( n = 0 ; n < this->numEq ; n++ )
    {
        if ( ! this->equations[n].endOfRow )
        {
            ...
            *Dnext = *H;
            ...
            // printf("dx left and right : %10.4fd %10.4f\n",
            // DxDyLast,DxDyFirst );
            // Log(LOG_ITER_MAJOR, "solution couple pt. right dom.: %10.5e %10.5e",*D,*H);
            printf("solution couple pt. right dom.: %10.5e %10.5e\n",*D,*H);
#endif
        }
    }
}
...
void WangEquations::AddEquationSpace(void)
{
    ...
    if ( newEquations == NULL )
    {
		...
		Abort("Unable to realloc in WangEquations::AddEquationSpace\n");
	}
