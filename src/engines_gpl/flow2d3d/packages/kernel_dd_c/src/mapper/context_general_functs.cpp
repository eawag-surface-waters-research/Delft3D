...
int D3dFlowContext::GetContextId(void)
{
    ...
    {
        Abort ("Context not yet Initialized" );
    }
    ...
    )
{
    ...
    {
        Abort ("Trying to attach var. to wrong context");
    }
    else
    {
        ...
        if ( ptr == NULL )
        {
            Abort ("Var (%s) not attached to Context (%d)", name, aContextId);
        }
        else
        {
            *var = ptr;
