...
//
// Functions for U/V/Zeta mapping
//
    ...
    )
{
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        MAP_CELLS_LOOP(ctx,eq)
        {
            ...
            default:
                Abort(
                (char*)"unexpected case (%d) InitKc", eq);
            }
        }
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        MAP_CELLS_LOOP(ctx,eq)
        {
            ...
            if ( Edge[ctx] == Edge_Left )
            {
                ...
                {
                    Abort( (char*)"%s\"%s\": Wrong coupling point at (%2d,%2d) Kcs=%d KcsInside=%d",
        ...
        for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
        {
            ...
            case Eq_Zeta:
                ...
                MAP_CELLS_LOOP(ctx,eq)
                {
                    ...
                    Log(LOG_ITER_MAJOR, "copy-dps: (%2d,%2d) %10.5f\n", m, n,cI2D(ctx,m,n,dps));
    ...
    )
{
    ...
    switch(seq)
    {
        ...
        Warn(
            (char*)"unexpected case CopyFlowVariables");
        break;
...

{
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        default:
            Warn(
                (char*)"unexpected case CopyOuterNormalVelocities");
            break;
...

{
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        default:
            Warn(
                (char*)"unexpected case CopyOuterTangentVelocities");
            break;
    ...
    )
{
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        default:
            ...
            Abort( (char*)"unexpected case BuildDdEquations");
            break;
        }
    }
}
    ...
    )
{
    ...
    Log(LOG_ITER_MAJOR, "MAPPER \"%s\" calls minimumbarrier (converged %d)", IteratorSelf()->Name(), converged);
    allConverged = MinimumBarrier(converged);
    Log(LOG_ITER_MAJOR, "MAPPER \"%s\" minimumbarrier returned, conv: %d", IteratorSelf()->Name(), allConverged);
...
int D3dFlowMapper::CheckForDry()
{
    ...
    if ( nMess != 2 )
    {
        Abort( (char*)"Didn't receive 2 dry messages");
    }

    Log(LOG_ITER_MAJOR, "MAPPER \"%s\" calls minimumbarrier (myWet %d)", IteratorSelf()->Name(), myWet);
    allWet = MinimumBarrier(myWet);
    Log(LOG_ITER_MAJOR, "MAPPER \"%s\" minimumbarrier returned (allWet %d)", IteratorSelf()->Name(), allWet);
    ...
    )
{
    ...
    for ( ctx = 0 ; ctx < NR_CNTXTS ; ctx++ )
    {
        ...
        default:
        Warn(
            (char*)"unexpected case CopySediment");
        break;
        }
    }
}
