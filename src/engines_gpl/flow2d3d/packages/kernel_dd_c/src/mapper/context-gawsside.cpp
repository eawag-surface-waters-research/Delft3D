    ...
    )
{
    ...
    Log(LOG_ITER_MAJOR, "GAWS \"%s\" WAITS for cID from flow \"%s\"",
    ...
    {
        Abort("D3dFlowContextGawsSide::Setup: unexpected blobID (%d /= %d)",
                    incomingBlobID, F2G_Blob_ContextID);
    }
    Log(LOG_ITER_MAJOR, "GAWS \"%s\" RECEIVED cID from flow \"%s\"",
                    this->gawsIterator->Name(), this->flowIterator->Name());

    if ( this->contextID == YET_TO_INIT )
    {
        Abort("GAWS didn't receive valid contextID from D3dFlow \"%s\"",
    ...
    {
        Log (LOG_ITER_MAJOR, "Context (%d) connected", this->contextID);
    ...
    else
    {
        ...
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" WAITS for KCS (%d*%d) from flow \"%s\"",
        ...
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" RECEIVED KCS from flow \"%s\"",
                        this->gawsIterator->Name(), this->flowIterator->Name());
    ...
    )
{
    ...
    Log(LOG_ITER_MAJOR, "D3dFlowContextGawsSide::Get_Adress for %d in \"%s\"(%s %2d,%2d): %x, value %12.5f",
...
void D3dFlowContextGawsSide::SetupCommunication(void)
{
    ...
    if ( this->memType == Mem_Distributed )
    {
        ...
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" SENDS numCommPoints to flow \"%s\"",
                        this->gawsIterator->Name(), this->flowIterator->Name());
        ...
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" HAS SENT numCommPoints (%d) to flow \"%s\"",
                        this->gawsIterator->Name(), this->numCommPoints, this->flowIterator->Name());

        if (this->numCommPoints > 0)
        {
            Log(LOG_ITER_MAJOR, "GAWS \"%s\" SENDS commPoints to flow \"%s\"",
                            this->gawsIterator->Name(), this->flowIterator->Name());
            ...
            Log(LOG_ITER_MAJOR, "GAWS \"%s\" HAS SENT commPoints to flow \"%s\"",
                            this->gawsIterator->Name(), this->flowIterator->Name());
        }
    }
}
...
void D3dFlowContextGawsSide::UpdateGawsToFlow(GawsDistribGroup distribGroup)
{
    ...
    if ( this->memType == Mem_Distributed )
    {
        ...
        {
            Abort("UpdateGawsToFlow: flowIterator not set for Gaws \"%s\"",
        ...
        else
        {
            ...
            if ( ( this->numCommPoints > 0 ) && ( distribGroup == GawsDistrib_AllVars ) )
            {
                ...
                Log(LOG_ITER_MAJOR, "GAWS \"%s\" SENDS Update to flow \"%s\"",
                                this->gawsIterator->Name(), this->flowIterator->Name());
                ...
                Log(LOG_ITER_MAJOR, "GAWS \"%s\" HAS SENT Update to flow \"%s\"",
                                this->gawsIterator->Name(), this->flowIterator->Name());
...
void D3dFlowContextGawsSide::UpdateGawsFromFlow(GawsDistribGroup distribGroup)
{
    ...
    if ( this->memType == Mem_Distributed )
    {
        if ( this->flowIterator == NULL )
        {
            Abort("UpdateGawsFromFlow: flowIterator not set for Gaws \"%s\"",
                this->gawsIterator == NULL ? "????" : this->gawsIterator->Name() );
        }
        else
        {
            ...
            Log(LOG_ITER_MAJOR, "GAWS \"%s\" WAITS for Update from flow \"%s\"",
                            this->gawsIterator->Name(), this->flowIterator->Name());
            ...
            {
                Abort("D3dFlowContextGawsSide::UpdateGawsFromFlow: unexpected blobID (%d /= %d)",
                            incomingBlobID, F2G_Blob_Update);
            }

            Log(LOG_ITER_MAJOR, "GAWS \"%s\" RECEIVED Update from flow \"%s\"",
                            this->gawsIterator->Name(), this->flowIterator->Name());
        ...
        else
        {
            if ( ( this->numCommPoints % GAWS_COMM_POINTS_BLOCKSIZE ) == 0 )
            {
                ...
                if ( ! allreadyThere )
                {
                        ...
                        Log(LOG_ITER_MAJOR, "AddedCommPoint in \"%s\": %3d,%3d (type=%d)",
...
void D3dFlowContextGawsSide::ReceiveSizesFromFlow(void)
{
    MAPDBG_FUN2("D3dFlowContextGawsSide::ReceiveSizesFromFlow");
        ...
        if ( this->flowIterator == NULL )
    {
        Abort("ReceiveSizesFromFlow: flowIterator not set for Mapper \"%s\"",
            this->gawsIterator == NULL ? "????" : this->gawsIterator->Name() );
    }
    else
    {
        ...
        Log(LOG_ITER_MAJOR, "GAWS \"%s\" WAITS for sizes from flow \"%s\"",
                        this->gawsIterator->Name(), this->flowIterator->Name());
        ...
        {
            Abort("D3dFlowContextGawsSide::ReceiveSizesFromFlow: unexpected blobID (%d /= %d)",
                        incomingBlobID, F2G_Blob_SubdomainSizes);
        }

        Log(LOG_ITER_MAJOR, "GAWS \"%s\" RECEIVED sizes from flow \"%s\"",
                        this->gawsIterator->Name(), this->flowIterator->Name());
        ...
        Log(LOG_ITER_MAJOR, "\"%s\"<-\"%s\": mMx=%d, nMx=%d, nMxus=%d, kMx=%d, dDb=%d, mMxdb=%d, nMxdb=%d",
                        this->gawsIterator->Name(), this->flowIterator->Name(),
    ...
    )
{
    ...
    for ( i = 0 ; i < this->numCommPoints ; i++ )
    {
        if ( bufferAction == BufferAction_Read )
        {
            * (var + i) = *copyTarget++;
            ON_LOG_GAWS_EXCHANGE(
                Log(LOG_ITER_MAJOR, "GAWSBufferComm Read from \"%10s\" %x (%3i): %10.4f",
        ...
        else
        {
            *copyTarget++ = * (var + i);
            ON_LOG_GAWS_EXCHANGE(
                Log(LOG_ITER_MAJOR, "GAWSBufferComm Fill for  \"%10s\" %x (%3i): %10.4f",
                        this->flowIterator->Name(), var, i, * (var + i)  );
                fflush(stdout);
                )
        }
