...
!
    ...
    real(fp), dimension(gdp%d%nmlb:gdp%d%nmub)        , intent(in) :: s00 !  Description and declaration in rjdim.f90
    ...
    real(fp), dimension(gdp%d%nmlb:gdp%d%nmub, kmax)               :: p00 !  Description and declaration in rjdim.f90
...
!
       ...
       do nm = nmst, nmst + ndelta
          if (kcs(nm)==1) then
             ...
             do k = kfsmin(nm), kfsmax(nm)
                ...
                dyd  = guv(ndm)
                dy   = guz(nm)
                dzup = .5*(dzs1(nm, k) + dzs1(nm, ku))
                dzd  = .5*(dzs1(nm, k) + dzs1(nm, kd))
                aak2(nm, k) = -dzv0(ndm, k)*dt/(dy*dyd*rhow)*kfvz1(ndm, k)      &
                            & *kfsz1(ndm, k)
                cck2(nm, k) = -dzv0(nm, k)*dt/(dy*dyu*rhow)*kfvz1(nm, k)        &
                            & *kfsz1(num, k)
                bbka(nm, k) = -dt/(dzd*rhow)
                bbkc(nm, k) = -dt/(dzup*rhow)
                aak(nm, k) = -dzu0(nmd, k)*dt/(dx*dxd*rhow)*kfuz1(nmd, k)       &
                           & *kfsz1(nmd, k)
                cck(nm, k) = -dzu0(nm, k)*dt/(dx*dxu*rhow)*kfuz1(nm, k)         &
                           & *kfsz1(nmu, k)
                bbk(nm, k) = -aak(nm, k) - cck(nm, k) - aak2(nm, k)             &
                           & - cck2(nm, k) - bbka(nm, k) - bbkc(nm, k)
                ddk(nm, k) = -(u1(nm, k)*dzu0(nm, k) - u1(nmd, k)*dzu0(nmd, k)) &
                           & /dx - (v1(nm, k)*dzv0(nm, k) - v1(ndm, k)          &
                           & *dzv0(ndm, k))/dy - w1(nm, k) + w1(nm, k - 1)
             enddo
          endif
       enddo
    enddo
    ...
    ! s00 is the waterlevel at the end of the previous time-step (set in momcor.f, not in f0isf1.f!)
    !
    do m = m1_nhy, m2_nhy
       nmst = nmstart + (m - m1_nhy)*icxy
       do nm = nmst, nmst + ndelta
          ...
          nmd = nm - icx
          ndm = nm - icy
          if (kfsmin(nm) <= kfsmax(nm) .and. kcs(nm)==1) then
             !
             ! For bottom layer:
             !
             k           = kfsmin(nm) !! adaptation 11/1/2007
             bbka(nm, k) = 0.
             bbk(nm, k)  = -aak(nm, k) - cck(nm, k) - aak2(nm, k)          &
                         & - cck2(nm, k) - bbkc(nm, k)
             !
             ! For top layer:
             !
             k           = kfsmax(nm) !! adaptation 11/1/2007
             dx          = .5*(gvv(nm) + gvv(ndm))
             dy          = .5*(guu(nm) + guu(nmd))
             bbkc(nm, k) = 0.
             bbk(nm, k)  = -aak(nm, k) - cck(nm, k) - aak2(nm, k)          &
                         & - cck2(nm, k) - bbka(nm, k) + 1./(dt*ag*rhow)
             ddk(nm, k)  = -(u1(nm, k)*dzu0(nm, k) - u1(nmd, k)*dzu0(nmd, k&
                         & ))/dx - (v1(nm, k)*dzv0(nm, k) - v1(ndm, k)     &
                         & *dzv0(ndm, k))/dy + w1(nm, k - 1) - s1(nm)      &
                         & /dt + s00(nm)/dt
          endif
       enddo
    enddo
    ...
    do i = 1, nsrc
       ...
       if (kcshyd(nm) ==1) then
          ...
          !        if (kfs(nm).ne.0 .or. disch(i).ge.0.0) then
