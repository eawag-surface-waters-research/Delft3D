...
!
    ...
    real(fp)  , dimension(gdp%d%nmlb:gdp%d%nmub, 0:kmax)                    :: rich   !  Description and declaration in rjdim.f90
       ...
       if (ltur==0) then
          do k = 1, kmax
             ...
             do nm = 1, nmmax
                ...
                if (kfs(nm)==1) then
                   ...
                   uuu = 0.5*maskval*sqrt((u0(nm, k) + u0(nmd, k))              &
                       & **2 + (v0(nm, k) + v0(ndm, k))**2)
                   !
          ...
          do nm = 1, nmmax
             ...
             if (kfs(nm)==1) then
                ...
                uuu = 0.5*maskval*sqrt((u0(nm, kmax) + u0(nmd, kmax))           &
                    & **2 + (v0(nm, kmax) + v0(ndm, kmax))**2)
                !
                ! bottom is assumed at Z0
                !
