subroutine z_sud(j         ,nmmaxj    ,nmmax     ,kmax      ,mmax      , &
               ...
               & flood     ,norow     ,irocol    ,mnksrc    , &
               & kfu       ,kfv       ,kfs       ,kcs       ,kcu       , &
               & kfuz1     ,kfvz1     ,kfsz1     ,kspu      ,kcs45     , &
               & kcscut    ,kfumin    ,kfumax    ,kfsmin    ,kfsmax    , &
               & kfumx0    ,thick     ,circ2d    ,circ3d    ,s0        , &
               & s1        ,u0        ,u1        ,v1        ,w1        , &
               & qxk       ,qyk       ,qzk       ,guu       ,gvv       , &
               & guv       ,gvu       ,gud       ,gvd       ,guz       , &
               & gvz       ,gsqiu     ,gsqs      ,disch     ,umdis     , &
               & dismmt    ,umean     ,evap      ,hu        ,dps       , &
               & dpu       ,dzu1      ,dzs1      ,dzu0      ,a         , &
               & b         ,c         ,d         ,aa        ,bb        , &
               & cc        ,dd        ,tetau     ,aak       ,bbk       , &
               & cck       ,ddk       ,d0        ,d0k       ,bbka      , &
...
!
    ...
    integer    , dimension(gdp%d%nmlb:gdp%d%nmub)                 :: kcscut !  Description and declaration in iidim.f90
    ...
    real(fp), dimension(gdp%d%nmlb:gdp%d%nmub)                    :: cc     !!  Internal work array, coupling mean
                                                                            ...
                                                                            !!  right (upper)
    real(fp)     , dimension(gdp%d%nmlb:gdp%d%nmub, 3)            :: cfurou !  Description and declaration in rjdim.f90 !!  of the momentum eq. at (N,M)
...
!
             ...
             do kk = kfsmin(nm), kfsmax(nm)
                ...
                if (kfsmin(nm) == kfsmax(nm)) then
                   ...
                   d0k(nm, kk) = d0k(nm, kk) + disch(i)*dzs1(nm, kk)               &
                               & /( max((real(dps(nm),fp) + s1(nm)), 0.01_fp)*gsqs(nm) )
                endif
             enddo
          endif
       else
    ...
    do nm = 1, nmmax
       do k = kfumin(nm), kfumax(nm)
          fac    = dzu1(nm, k)*porosu(nm, k)/(bbk(nm, k)*hu(nm))
    ...
    !
    ! d0k on layers where kfvz1 unequal to 0
    !
    call timer_start(timer_sud_rest, gdp)
    do nm = 1, nmmax
       if (kcs(nm)/=0) then
 ...
 9999 continue
    ...
    do nm = 1, nmmax
       nmd = nm - icx
       if (kcs(nm)==1) then
          dxid  = hu(nmd)*guu(nmd)/gsqs(nm)
          dxiu  = hu(nm)*guu(nm)/gsqs(nm)
          a(nm) = aa(nmd)*dxid
       ...
       if (kfu(nm)==1) then
          do k = kfumin(nm), kmax
             if (k<=kfumax(nm)) then
                ...
                qxk(nm, k) = dzu1(nm, k)*u1(nm, k)*guu(nm)*porosu(nm, k)
             else
                u1(nm, k) = 0.
                qxk(nm, k) = 0.
             endif
