...
proc startMormerge { inputfilename workdir mergeexe localrun runid node } {
   ...
   if { [string equal $platform "linux"] } {
      ...
      puts $shellscriptfile "\nrem This script file is automatically generated by mormerge.tcl, version $version\n"
      puts $shellscriptfile "start $sfilname\n"
   }
...
proc startFlow { inflist alist condition runids waveonline tdatomexe flowexe waveexe node } {
   ...
   if { [string equal $platform "linux"] } {
      ...
      puts $scriptfile "\n# Set some environment parameters\n"
      if { $arglist(testbank) } {
         set archpos [string first $arch $flowexe]
         puts $scriptfile "export D3D_HOME=\"[string range $flowexe 0 [expr $archpos-1]]\""
      } else {
      ...
      if {$queuesys != "sge"} {
         # puts $scriptfile "export LD_ASSUME_KERNEL=2.4.19"
      }
      # puts $scriptfile "export LD_LIBRARY_PATH=\"/opt/intel_cc_90/lib:/opt/intel_fc_90/lib:/opt/jdk1.5/jre/lib/i386:/opt/jdk1.5/jre/lib/i386/client:/u/elshoff/DOL/DelftOnline/lib:/u/elshoff/DOL/JavaLaunch/lib\""
      # for SWAN:
      ...
      if { $arglist(testbank) } {
         set archpos [string first $arch $flowexe]
         puts $scriptfile "set D3D_HOME=[file nativename [string range $flowexe 0 [expr $archpos-1]] ]"
      } else {
