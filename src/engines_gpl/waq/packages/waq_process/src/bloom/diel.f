...
!
!  *********************************************************************
!  *    SUBROUTINE TO CALCULATE DIURNAL PRODUCTION AND RESPIRATION     *
!  *                        RATES OF THE BLOOM                         *
!  *********************************************************************
!
...
!
!  Calculate production and respiration rates per hour during 6 daily
!  time intervals, the lenght which varies with the time of year.
!  For a more detailed distribution, the dimensions, DO-loops and
!  FORMATS of this subroutine should be changed.
!  All data are in mg O2 / m3 / hour, except "TOTAL".
!  It is assumed that equinox is at 12.00 hours sharp.
!  Normal output of the subroutine is to unit IOU(18), but
!  if option "OXMODOUT" is chosen, a specially formatted output will
!  be wrtitten to unit IOU(19) for the oxygen model "OXYMOD".
!
      IF (NPRODU .GT. 1) GO TO 40
!
!  print heading for output on unit IOU(18)
!
      CALL FORMFE (IOU(18))
      WRITE (IOU(18),10)
   ...
   20 FORMAT (//,1X,116('-'))
      LINEPA = 1
!
!  Print heading for output on unit IOU(19)
!
   ...
   40 CONTINUE
!
!  Establish time interval for this period
!
      DO 50 K=1,3
...
!
!  Calculate gross-, net-production and respiration according to
!  distribution 1 per hour for each interval.
!
      RHOUR=SUMROX/24.0
      DO 120 K=1,6
...
!
  ...
  160 FORMAT (' Net-Production',3X,']',2X,6(F6.1,3X,']',2X),8X,F7.1,2X,
     1         F7.1)
!
!  Optionally print output for "OXYMOD".
!
...
!
!  Calculate gross-, net-production and respiration according to
!  distribution 2 per hour for each interval.
!
      J=7
      PHORES=DAY*RHOUR
      DO 190 K=1,6
