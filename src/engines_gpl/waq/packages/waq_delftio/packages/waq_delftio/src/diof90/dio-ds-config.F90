...
end type waq_DioStConfigType


type waq_DioDsConfigType
    character(len=DioMaxPropLen)           :: name
...
end type waq_DioDsConfigType


type waq_DioConfigType
    character(len=DioMaxStreamLen):: iniFile
    type(waq_DioStConfigType)         :: settings
    type(waq_DioDsConfigType)         :: dsDefaults
    type(waq_DioDsConfigType),&
        dimension(MaxNrDsConfigs) :: dsConfig
    integer                       :: nDs            ! # dataset configs
end type waq_DioConfigType


type(waq_DioConfigType), private, target :: dioConfig
...
interface waq_DioInit
    module procedure waq_DioInit
...
interface waq_DioConfGetStProp
...
interface waq_DioConfGetDsProp
...
interface waq_DioSetDefaultDsProp
...
subroutine DioConfigDump(lun)
    ...
    integer                        :: lun
    integer                        :: d
    type(waq_DioDsConfigType), pointer :: ds
...
subroutine DioReadDsConfig(ds, group)
    type(waq_DioDsConfigType)          :: ds
    character(Len=*)                  :: group

    character(len=DioMaxPropLen)   :: valString
    logical                        :: getRes

    getRes = waq_prop_get( group, 'Name'        , ds % name )
    getRes = waq_prop_get( group, 'OnLine'      , ds % logProp(dio_ds_OnLine) )
    getRes = waq_prop_get( group, 'Active'      , ds % logProp(dio_ds_Active) )
    getRes = waq_prop_get( group, 'StartTime'   , ds % strProp(dio_ds_StartTime) )
    getRes = waq_prop_get( group, 'DeltaTime'   , ds % strProp(dio_ds_DeltaTime) )
    getRes = waq_prop_get( group, 'HisTimeUnit' , ds % intProp(dio_ds_HisTimeUnit) )

    if ( waq_prop_get( group, 'StreamType', valString ) ) then
        ds % intProp(dio_ds_StreamType) = DetermStreamType(valString)
    endif

    if ( waq_prop_get( group, 'VarType', valString ) ) then
...
subroutine DioReadStConfig
    ...
    type(waq_DioStConfigType), pointer :: settings        ! pointer to settings part of config
    ...
    getRes = waq_prop_get( 'Settings', 'Delay'    , settings % intProp(dio_st_Delay) )
    getRes = waq_prop_get( 'Settings', 'SleepTime', settings % intProp(dio_st_SleepTime) )
    getRes = waq_prop_get( 'Settings', 'TimeOut'  , settings % strProp(dio_st_TimeOut) )
...
subroutine DioReadIniFile(fileName)
    ...
    getRes = waq_prop_get( group, 'NumberOfDatasets', dioConfig % nDs )
    do d = 1, dioConfig % nDs
        if ( d .lt. 10 ) then
