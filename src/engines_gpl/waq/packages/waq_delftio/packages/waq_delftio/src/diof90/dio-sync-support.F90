...
#endif
        if (ierr.gt.0) then
            call waq_DioStreamError(901, 'Could not set available:', fname)
        else
            close(iunit)
        endif
        call DioStreamDelay
...
subroutine DioSyncSetItemReceived(stream, itemName, availType)
    ...
    if ( stream % synched ) then
        ...
        open(newunit=iunit,file=fname,status='old', iostat=ierr)
        if (ierr.gt.0) then
            call waq_DioStreamError(902, 'Could not open ', trim(fname), ' for deletion')
        else
            ...
            if ( availType .ne. '.stream' .and. DioStreamUsesLun(stream)) then
                ...
                open(newunit=iunit,file=fname,status='old', iostat=ierr)
                if (ierr.gt.0) then
                    call waq_DioStreamError(903, 'Could not open ', trim(fname), ' for deletion')
                else
                    close(iunit,status='delete')
                endif
            endif
