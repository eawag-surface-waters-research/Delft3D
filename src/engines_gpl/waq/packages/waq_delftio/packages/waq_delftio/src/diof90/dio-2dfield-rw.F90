...
type waq_Dio2DFType
    ...
    type(waq_Dio2DFHeaderType)        :: header      ! dataset header
    type(waq_Dio3DType)               :: values      ! dataset values
end type waq_Dio2DFType
...
function Dio2DFCheckSizes(f2D, nP, nL, modus) result(ok)
    ...
    type(waq_Dio2DFType), intent(in)  :: f2D              ! dataset
...
function Dio2DFCreateEmpty(name, varType) result(f2D)
    ...
    type(waq_Dio2DFType)             :: f2D      ! dataset
...
function waq_Dio2DFCreate(name, varType, numM, numN) result(f2D)
...
subroutine Dio2DFWriteHeader(f2D)
    ...
    type(waq_Dio2DFType), target        :: f2D                    ! dataset

    ! locals

    type(waq_Dio2DFHeaderType), pointer :: header                 ! dataset header
...
#else
                    call waq_DioStreamError(391, 'Nefis not supported (ds: ', f2D % ds % name, ')')
#endif
        ...
        if (DioSyncDsItemAvailable(f2D % ds)) then
            if ( DioStreamConnect(f2D % ds % inStream, .false. ) ) then
                ...
                if ( sType .eq. dio_Nefis_Stream ) then
                    ...
                    retVal = .true.
                    goto 992
                991 call waq_DioStreamError(301, 'Error reading ASCII/BIN Header:', f2D % ds % name)
                992 continue
...
function waq_Dio2DFDefine(stream, name, varType, numM, numN) result(f2D)
    ...
    f2D = waq_Dio2DFCreate(name, varType, numM, numN)

    f2D % ds % outStream = stream

    call Dio2DFWriteHeader(f2D)

end function waq_Dio2DFDefine
...
function Dio2DFDefineAutoStream(name, varType, numM, numN) result(f2D)
    ...
    f2D = waq_Dio2DFDefine(stream, name, varType, numM, numN)
...
subroutine Dio2DFPutReals(f2D, values)
    ...
    call DioDsIncreaseTimestep(f2D % ds)
    call waq_Dio2DFSetValues(f2D, values)
...
function waq_Dio2DFGetDataset(stream, name) result(f2D)
    ...
    f2D = waq_Dio2DFCreate(name, Dio_Plt_Unknown)
    f2D % ds % inStream = stream

    call Dio2DFReadHeader(f2D)

end function waq_Dio2DFGetDataset
...
function Dio2DFGetDatasetAutoStream(name) result(f2D)
    ...
    f2D = waq_Dio2DFGetDataset(stream, name)
