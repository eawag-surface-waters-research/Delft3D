...
function DioCreateStream(streamType, name, mode) result(streamHandle)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DioCreateStream
    ...
    type(waq_DioStreamType), pointer          :: stream
...
function DioDefinePltDataSet(streamHandle, name, varType,&
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DioDefinePltDataSet
    ...
    ! locals
    type(waq_DioStreamType), pointer   :: stream         ! handle to stream
    ...
    if ( .not. associated(stream) ) then
        call waq_DioStreamError(403, 'DioDefinePltDataSet: invalid streamHandle')
    else
        if ( .not. stream % opened ) then
            call waq_DioStreamError(404, &
                'DioDefinePltDataSet: stream not open', stream % name)
        else
            pltHandle = DioPltNewF77Handle()
            if ( pltHandle > 0 ) then
                PltDataset(pltHandle) = waq_DioPltDefine(stream, name, varType, pars, locs)
...
subroutine DioPutPltDataSetReals(pltHandle, timestep, nPar, nLoc, values)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DioPutPltDataSetReals
    ...
    type(waq_DioPltType), pointer         :: plt
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        ...
        if ( (npar .ne. plt % header % nPar) .or. &
            ...
            call waq_DioStreamError(401, plt % ds % name, &
                ': DioPutPltDataSetReals: wrong sizes for time ', timestep)
        else
            call waq_DioPltPut(plt, values)
        endif
    endif
...
function DioGetPltDataSetInfo(streamHandle, name, npar, pars,&
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DioGetPltDataSetInfo
    ...
    ! locals
    type(waq_DioPltType), pointer    :: plt            ! f90 dataset handle
    type(waq_DioStreamType), pointer :: stream         ! stream handle
...
function DioGetPltDataSetReals(pltHandle, timestep, npar, nloc, values) result(retVal)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DioGetPltDataSetReals
    ...
    ! locals
    type(waq_DioPltType), pointer         :: plt       ! F90handle to PLT
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        plt => PltDataset(pltHandle)
        retVal = waq_DioPltGet(plt, locValues)
        if ( retVal ) then
            if ( (npar .lt. plt % header % nPar) .or. &
                 (nloc .lt. plt % header % nLoc)      ) then
                call waq_DioStreamError(401, 'DioGetPltDataSetReals: wrong sizes ',&
...
function DiofPltDefine_1(name, varType, npar, pars, nloc, locs) result(pltHandle)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_1
    ...
    pltHandle = DioPltNewF77Handle()
    if ( pltHandle > 0 ) then
        PltDataset(pltHandle) = waq_DioPltDefine(name, varType, pars, locs)
        PltDataset(pltHandle) % ds % id = pltHandle
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_2a
    ...
    if ( pltHandle > 0 ) then
        if ( endTime > startTime ) then
            PltDataset(pltHandle) = waq_DioPltDefine(name, varType, pars, locs, startTime, endTime)
        else
            PltDataset(pltHandle) = waq_DioPltDefine(name, varType, pars, locs, startTime)
        endif
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
...
function DiofPltDefine_3(name, runid, varType, &
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_3
    ...
    pltHandle = DioPltNewF77Handle()
    if ( pltHandle > 0 ) then
        PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, locs)
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
             pltHandle = 0
...
function DiofPltDefine_4a(name, runid, varType, &
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_4a
    ...
    if ( pltHandle > 0 ) then
        if ( endTime > startTime ) then
            PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, locs, startTime, endTime)
        else
            PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, locs, startTime)
        endif
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
...
function DiofPltDefine_5a(name, runid, varType, &
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_5a
    ...
    if ( pltHandle > 0 ) then
        if ( endTime > startTime ) then
            PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, intIds, locs, startTime, endTime)
        else
            PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, intIds, locs, startTime)
        endif
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
...
function DiofPltDefine_11(name, varType, npar, pars, nLoc) result(pltHandle)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_11
    ...
    pltHandle = DioPltNewF77Handle()
    if ( pltHandle > 0 ) then
        PltDataset(pltHandle) = waq_DioPltDefine(name, varType, pars, nLoc)
        PltDataset(pltHandle) % ds % id = pltHandle
        if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
             pltHandle = 0
...
function DiofPltDefine_12a(name, varType, npar, pars, nLoc, startTime, endTime) result(pltHandle)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_12a
    ...
    if ( pltHandle > 0 ) then
        if ( endTime > startTime ) then
            PltDataset(pltHandle) = waq_DioPltDefine(name, varType, pars, nLoc, startTime, endTime)
        else
            PltDataset(pltHandle) = waq_DioPltDefine(name, varType, pars, nLoc, startTime)
        endif
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
...
function DiofPltDefine_13(name, runid, varType, &
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_13
    ...
    pltHandle = DioPltNewF77Handle()
    if ( pltHandle > 0 ) then
        PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, nLoc)
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
             pltHandle = 0
...
function DiofPltDefine_14a(name, runid, varType, &
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltDefine_14a
    ...
    if ( pltHandle > 0 ) then
        if ( endTime > startTime ) then
            PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, nLoc, startTime, endTime)
        else
            PltDataset(pltHandle) = waq_DioPltDefine(name, runId, varType, pars, nLoc, startTime)
        endif
        PltDataset(pltHandle) % ds % id = pltHandle
            if (.not. DioPltOpenedOK(PltDataset(pltHandle)) ) then
             call DioPltReleaseF77Handle(pltHandle)
...
subroutine DiofPltPutFloats(pltHandle, timestep, nPar, nLoc, values)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltPutFloats
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        plt => PltDataset(pltHandle)
        call waq_DioPltPut(plt, timestep, values)
    endif
...
function DiofPltGetNPars(pltHandle) result(retVal)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltGetNPars
    ...
    ! locals
    type(waq_DioPltType), pointer :: plt       ! pointer to PLT
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        ...
        if (associated(plt % header % locs) ) then
            ...
            if ( DioPltGetStreamType(plt) .eq. Dio_WQMap_stream ) then
                ...
                call waq_DioStreamError(406, 'DiofPltGetLocs: Loc names not available', &
                                           plt % ds % name)
            endif
        endif
    endif
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        ...
        if (associated(plt % header % hisIntIds) ) then
            ...
            if ( DioPltGetStreamType(plt) .eq. Dio_WQMap_stream ) then
                ...
                call waq_DioStreamError(406, 'DiofPltGetIntIds: Int ids not available',&
                                             plt % ds % name)
            endif
        endif
    endif
        ...
        if ( nTimInPlt >= nTim ) then
            ...
            if (associated(locTimes)) then
                ...
                if ( nTim .eq. 0 ) then
                    ...
                    call waq_DioStreamError(408, &
                        'DiofPltGetTimes: No times available', plt % ds % name)
                endif
            endif
        else
            call waq_DioStreamError(407, &
...
function DiofPltGetNextFloats(pltHandle, npar, nloc, timestep, values) result(retVal)
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltGetNextFloats
    ...
    ! locals
    type(waq_DioPltType), pointer       :: plt       ! pointer to actual PLT
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        plt => PltDataset(pltHandle)
        retVal = waq_DioPltGet(plt, timestep, locValues)
        if ( retVal ) then
            if ( (npar .lt. plt % header % nPar) .or. &
                 (nloc .lt. plt % header % nLoc)      ) then
                call waq_DioStreamError(401, 'DiofPltGetNextFloats: wrong sizes ', plt % ds % name)
                retVal = .false.
            else
                do par = 1, plt % header % nPar
                    do loc = 1, plt % header % nLoc
  ...
  !DEC$ATTRIBUTES DLLEXPORT :: DiofPltGetNextDoubles
    ...
    if ( DioPltF77HandleIsValid(pltHandle) ) then
        ...
        if ( retVal ) then
            if ( (npar .lt. plt % header % nPar) .or. &
                 (nloc .lt. plt % header % nLoc)      ) then
                call waq_DioStreamError(401, 'DiofPltGetNextDoubles: wrong sizes ', plt % ds % name)
                retVal = .false.
            else
                do par = 1, plt % header % nPar
                    do loc = 1, plt % header % nLoc
