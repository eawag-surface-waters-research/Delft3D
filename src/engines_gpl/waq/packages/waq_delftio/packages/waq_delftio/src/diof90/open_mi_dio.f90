...
! Overloaded Functions
!
    interface waq_OD_ExchItemCreate
...
!
    ...
    type waq_t_od_exchange
        ...
        type(waq_DioPltType)               :: exchPlt    ! DelftIO PLT for shared mem. exchange
        type(waq_DioPltType)               :: dumpPlt    ! DelftIO PLT for dumping values

    end type waq_t_od_exchange
    !
    ...
    end type waq_t_od_store
!
! Module variables
!
    type(waq_t_od_store)  , target  :: odStore                               ! Exchanged items store
...
function OD_ExchItemCreate_WithIDs(quantID, elmsetID, elmIDs, role, startTime, dumpDsName) result(success)
    ...
    !
    ! locals
    type(waq_t_od_exchange), pointer             :: od_exchange ! pointer to exchanged item
    ...
    if ( role /= od_providing ) then
        ...
        if ( .not. associated(od_exchange) ) then
            ...
            arrQuant(1) = quantID
            od_exchange % exchPlt = waq_DioPltDefine(od_exchange % exchDsName,      &
            ...
            if ( DioGetLastError() /= 0 ) then
                ...
                success = .true.
                if (dumpValues) then
                    od_exchange % dumpPlt = waq_DioPltDefine(od_exchange % dumpDsName,      &
                                                 dio_plt_real, arrQuant,  elmIDs, startTime )
                endif
            endif
        endif
...
function OD_ExchItemCreate_NoIDs(quantID, elmsetID, role) result(success)
    ...
    !
    ! locals
    type(waq_t_od_exchange), pointer :: od_exchange
    !
    ! body
    success = .false.
    if ( role /= od_accepting ) then
        ...
        if ( .not. associated(od_exchange) ) then
            ...
            od_exchange % exchPlt = waq_DioPltGetDataset(od_exchange % exchDsName)
            if ( DioGetLastError() /= 0 ) then
                write(odLastError,'(A)') 'OPENMI_DIO: ERROR 100: See dio error file'
            else
                success = .true.
...
function OD_Put_1D(julianTime, quantID, elmsetID, values) result(success)
    ...
    if ( .not. associated(od_exchange) ) then
        ...
        if ( .not. DO_FakeComm() ) then
            call waq_DioPltPut(od_exchange % exchPlt, julianTime, values2D)
        endif
        if (dumpValues) then
            call waq_DioPltPut(od_exchange % dumpPlt, julianTime, values2D)
        endif
        deallocate(values2D)
        success = .true.
    endif
...
function OD_Put_2D(julianTime, quantID, elmsetID, values) result(success)
    ...
    if ( .not. associated(od_exchange) ) then
        ...
        if ( .not. DO_FakeComm() ) then
            call waq_DioPltPut(od_exchange % exchPlt, julianTime, values)
        endif
        if (dumpValues) then
            call waq_DioPltPut(od_exchange % dumpPlt, julianTime, values)
        endif
        success = .true.
    endif
...
function OD_Get_1D(quantID, elmsetID, values) result(success)
    ...
    if ( DO_FakeComm() ) then
        ...
        if ( .not. associated(od_exchange) ) then
            ...
            if ( .not. waq_DioPltGet(od_exchange % exchPlt, values2D) ) then
                    write(odLastError, '(4A)')                                    &
