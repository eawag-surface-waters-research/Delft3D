...
module waq_Dio_plt_rw

use waq_Dio_ini
use waq_Dio_ds
use waq_Dio_3d_block
...
type waq_DioPltType
    ...
    type(waq_DioPltHeaderType)        :: header      ! dataset header
    type(waq_Dio3DType)               :: values      ! dataset values
end type waq_DioPltType
...
interface waq_DioPltGetCurHisStep
    module procedure waq_DioPltGetCurHisStep
    module procedure DioPltGetCurHisStepAndUnit
end interface


interface waq_DioPltDefine
    module procedure waq_DioPltDefine
...
interface waq_DioPltGetDataset
    module procedure waq_DioPltGetDataset
    module procedure DioPltGetDatasetAutoStream
end interface


interface waq_DioPltGet
...
interface waq_DioPltGetSelection
...
type(waq_DioPltType), dimension(dioMaxPLTs), target :: PltDataset  ! datasets
...
function DioPltNewF77Handle() result(retVal)
    ...
    if ( retVal == 0 ) then
        call waq_DioStreamError(405, 'DioPltNewF77Handle: Out of PLT-handles')
    endif
...
function DioPltCheckSizes(plt, nP, nL, modus) result(ok)
    ...
    type(waq_DioPltType), intent(in)  :: plt              ! dataset
    ...
    if ( modus .eq. Dio_DsPltLarger ) then
        ...
        if ( .not. ok ) then
            call waq_DioStreamError(201, 'Size of PLT Array in PUT larger then NPAR/NLOC for ', plt % ds % name)
        endif
        ...
        if ( .not. ok ) then
            call waq_DioStreamError(202, 'Size of PLT Array in GET smaller then NPAR/NLOC for ', plt % ds % name)
        endif
    endif
...
subroutine DioPltWriteHeader(plt)
    ...
    type(waq_DioPltType), target        :: plt         ! dataset

    ! locals

    type(waq_DioPltHeaderType), pointer :: header      ! dataset header
    ...
    if ( plt % ds % outStream % opened ) then
        ...
        if ( DioSyncDsItemCanBeSent(plt % ds) ) then
...
#else
                        call waq_DioStreamError(291, 'Nefis not supported (ds: ', &
                                                plt % ds % name, ')')
#endif
                end select
...
subroutine DioPltReadHeader(plt)
    ...
    if ( plt % ds % inStream % opened ) then
        ...
        if (DioSyncDsItemAvailable(plt % ds)) then
...
!
function waq_DioPltCreate(name, varType, pars, locs) result(plt)
...
subroutine DioPltSetHisRunId(plt, runId, startTime, endTime)
    ...
    type(waq_DioPltType), target               :: plt       ! plt dataset
...
function DioPltGetHeaderLine(plt, lineNr, line) result(retVal)
    ...
    ! arguments
    type(waq_DioPltType), intent(in) :: plt      ! plt dataset
...
function DioPltGetCurrentTimeStep(plt) result(timeStep)
    ...
    ! arguments
    type(waq_DioPltType)              :: plt      ! plt dataset
...
!
function waq_DioPltGetCurHisStep(plt) result(hisStep)
...
end function waq_DioPltGetCurHisStep
...
!
function waq_DioPltDefine(stream, name, varType, pars, locs) result(plt)

    ! return value
    type(waq_DioPltType)                           :: plt     ! dataset

    ! arguments
    type(waq_DioStreamType), target, intent(in)    :: stream  ! stream
    ...
    ! body
    plt = waq_DioPltCreate(name, varType, pars, locs)
    plt % ds % outStream = stream
    call DioPltWriteHeader(plt)

end function waq_DioPltDefine
...
function DioPltDefineMap(stream, name, varType, pars, nLoc) result(plt)
    ...
    ! body
    plt = waq_DioPltCreate(name, varType, pars, nLoc)
    plt % ds % outStream = stream
    call DioPltWriteHeader(plt)
...
function DioPltDefAutoStream(name, varType, pars, locs) result(plt)
    ...
    plt = waq_DioPltDefine(stream, name, varType, pars, locs)
...
function DioPltDefAutoStreamMap(name, varType, pars, nLoc) result(plt)
    ...
    stream = waq_DioStreamCreate(Dio_WQMap_Stream, name, 'w')
    stream % autoStream = .true.
    plt = waq_DioPltDefine(stream, name, varType, pars, nLoc)
...
function DioPltDefAutoWithJulian(name, varType, pars, locs, startTime, endTime) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    if ( present(endTime) ) then
        plt = waq_DioPltDefine(stream, name, varType, pars, locs, startTime, endTime)
    else
        plt = waq_DioPltDefine(stream, name, varType, pars, locs, startTime)
    endif
...
function DioPltDefAutoWithJulianMap(name, varType, pars, nLoc, startTime, endTime) result(plt)
    ...
    stream % autoStream = .true.
    if ( present(endTime) ) then
        plt = waq_DioPltDefine(stream, name, varType, pars, nLoc, startTime, endTime)
    else
        plt = waq_DioPltDefine(stream, name, varType, pars, nLoc, startTime)
    endif
...
function DioPltDefAutoWithRunid(name, runId, varType, pars, locs) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    plt = waq_DioPltDefine(stream, name, runId, varType, pars, locs)
...
function DioPltDefAutoWithRunidMap(name, runId, varType, pars, nLoc) result(plt)
    ...
    stream = waq_DioStreamCreate(Dio_WQMap_Stream, name, 'w')
    stream % autoStream = .true.
    plt = waq_DioPltDefine(stream, name, runId, varType, pars, nLoc)
...
function DioPltDefAutoWithRIDAndJul(name, runId, varType, pars, locs, startTime, endTime) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    if ( present(endTime) ) then
        plt = waq_DioPltDefine(stream, name, runId, varType, pars, locs, startTime, endTime)
    else
        plt = waq_DioPltDefine(stream, name, runId, varType, pars, locs, startTime)
    endif
...
function DioPltDefAutoWithRIDAndJulMap(name, runId, varType, pars, nLoc, startTime, endTime) result(plt)
    ...
    stream % autoStream = .true.
    if ( present(endTime) ) then
        plt = waq_DioPltDefine(stream, name, runId, varType, pars, nLoc, startTime, endTime)
    else
        plt = waq_DioPltDefine(stream, name, runId, varType, pars, nLoc, startTime)
    endif
...
function DioPltDefAutoWithIntIds(name, varType, pars, intIds, locs) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    plt = waq_DioPltDefine(stream, name, varType, pars, intIds, locs)
...
function DioPltDefAutoWithRIDAndInts(name, runId, varType, pars, intIds, locs) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    plt = waq_DioPltDefine(stream, name, runId, varType, pars, intIds, locs)
...
function DioPltDefAutoWithRunidIntsJul1(name, runId, varType, pars, intIds, locs, startTime) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    plt = waq_DioPltDefine(stream, name, runId, varType, pars, intIds, locs, startTime)
...
function DioPltDefAutoWithRunidIntsJul2(name, runId, varType, pars, intIds, locs, startTime, endTime) result(plt)
    ...
    stream = DioStreamCreateAuto(name, 'w')
    plt = waq_DioPltDefine(stream, name, runId, varType, pars, intIds, locs, startTime, endTime)
...
subroutine DioPltPutOnStream(plt, stream)
    ...
    type(waq_DioPltType)                :: plt     ! dataset
    type(waq_DioStreamType), intent(in) :: stream  ! stream
...
subroutine DioPltPutReals(plt, values)
    ...
    call DioDsIncreaseTimestep(plt % ds)
    call waq_DioPltSetValues(plt, values)
...
subroutine DioPltPutHisStepReals1D(plt, intTime, values, allowLarger)
    ...
    if ( present(allowLarger) )then
        if (waq_DioPltSetValues1D(plt, values, allowLarger)) call DioPltWriteValues(plt)
    else
        if (waq_DioPltSetValues1D(plt, values)) call DioPltWriteValues(plt)
    endif
    ...
    if (plt % ds % curTimeIndex > 0) then
        ...
        if (tIndex < plt % ds % curTimeIndex) then
            ...
            if ( fseekResult .ne. 0 ) then
                call waq_DioStreamError(230, 'Could not jump back in his file')
            endif
...
!
function waq_DioPltGetDataset(stream, name) result(plt)
    ...
    plt = waq_DioPltCreate(name, Dio_Plt_Unknown)
    plt % ds % inStream = stream

    call DioPltReadHeader(plt)

end function waq_DioPltGetDataset
...
function DioPltGetDatasetAutoStream(name) result(plt)
    ...
    if (stream % opened) then
        plt = waq_DioPltGetDataset(stream, name)
    else
        call waq_DioStreamError(203, 'Could not open ', trim(name), ' for reading')
...
function DioPltGetHisStepReals(plt, hisStep, values) result(retVal)
    ...
    if (DioPltReadValues(Dio_Plt_Real, plt)) then
        values => plt % values % reals(:,:, plt % ds % curDataIndex )
        hisStep = waq_DioPltGetCurHisStep(plt)
    else
        call DioDsDecreaseTimestep(plt % ds)
    endif
...
subroutine DioPltHisWriteHeader(plt)
    ...
    lun = plt % ds % outStream % lun
    header => plt % header
    call waq_DioConfGetStProp(dio_st_AllowJumpBack, plt % header % allowJumpBack)
    if (plt % header % allowJumpBack) then
        plt % ds % timeGrows = .true.
    endif
...
subroutine DioPltHisWriteValues(plt, lun)
    ...
    if ( .not. associated(plt % values % reals) ) then
        if ( .not. associated(plt % values % doubles) ) then
            call waq_DioStreamError(223, 'ERROR in DioPltWriteValuesHis:', &
...
function DioPltHisReadHeader(plt) result(retVal)
    ...
    integer                         :: lun      ! stream lun
    type(waq_DioPltHeaderType), pointer :: header   ! dataset header
    type(waq_DioDsType), pointer        :: ds       ! dataset info
...
999     continue
        ...
        if ( plt % ds % inStream % synched ) then
            call waq_DioStreamError(224, 'ERROR in DioPltHisReadValues:', &
                        'Unexpected end of file')
        endif
    else
        call waq_DioStreamError(225, 'ERROR in DioPltHisReadValues:', &
...
subroutine DioPltHisReadAllTimes(plt)
    ...
    if ( fseek(plt % ds % inStream % lun,      &
               ...
               SEEK_SET                        ) .ne. 0 ) then
        call waq_DioStreamError(221, 'Could not Skip HIS header')
    else
    ...
    999     continue    ! error when reading time step
        ...
        if ( fseek(plt % ds % inStream % lun,      &
                   DioPltHisDetermineHeaderSize(plt), &
                   SEEK_SET                        ) .ne. 0 ) then
            call waq_DioStreamError(222, 'Could not ReSkip HIS header')
        endif
...
function DioPltHisGetSelectionReals(plt, nPar, parIndx, nLoc, locIndx,&
    ...
    if ( fseek(lun, headerSize, SEEK_SET) .ne. 0 ) then
        call waq_DioStreamError(226, 'Could not Skip HIS header')
    else
        ...
        if ( fseek(lun, headerSize, SEEK_SET ) .ne. 0 ) then
            call waq_DioStreamError(227, 'Could not ReSkip HIS header')
        endif
...
function HiaReadGeneralPart_V100(hiaFile, plt, dioVersionInFile) result(retVal)
    ...
    ! arguments
    type(waq_TDioIniFile)            :: hiaFile          ! HIA ini file
    type(waq_DioPltType), intent(IN) :: plt              ! PLT dataset
    ...
    if ( DioIniFindGroup(hiaFile, DioCheckGroup ) ) then
        ...
        if ( findRes ) then
            if ( nPar .ne. plt % header % nPar ) then
                call waq_DioStreamError(241, 'Incompatible #pars in HIA file for', &
                                                trim(plt % ds % name) )
                retVal = .false.
            endif
        endif
        ...
        if ( findRes ) then
            if ( nLoc .ne. plt % header % nLoc ) then
                call waq_DioStreamError(243, 'Incompatible #locs in HIA file for', &
                                                trim(plt % ds % name) )
                retVal = .false.
            endif
        endif
        ...
        if ( findRes ) then
            ...
            if ( DioT0inHIS .ne. DioT0inHIA ) then
                call waq_DioStreamError(245, 'Incompatible T0 in HIA file for', &
                                                trim(plt % ds % name) )
                retVal = .false.
            endif
        endif
        ...
        if ( findRes ) then
            ...
            if ( timeStepUnit .ne. plt % ds % hisTimeUnit ) then
                call waq_DioStreamError(247, 'Incompatible Time Step Unit in HIA file for', &
                                                trim(plt % ds % name) )
                retVal = .false.
            endif
        endif
        ...
        if ( findRes ) then
            if ( timeStepMult .ne. plt % ds % hisTimeMult) then
                call waq_DioStreamError(249, 'Incompatible Time Step Multiplier in HIA file for', &
                                                trim(plt % ds % name) )
                retVal = .false.
            endif
        endif
...
function HiaOverwriteFile(hiaName, doDelete) result(overWrite)
    ...
    ! locals
    type(waq_TDioIniFile)              :: hiaFile   ! HIA IniFile Handle
