...
handles(end+1)=uicontrol('style','text', ...
    'position',[HOffset VOffset-2 100 18], ...
    'string','GhostScript Location', ...
    'horizontalalignment','left', ...
    'parent',mfig);
VOffset = VOffset-20;
undefined = '--first-time--';
gs_cmd = qp_settings('ghostscript',undefined);
if strcmp(gs_cmd,undefined)
    gs_cmd = locate_ghostscript;
    qp_settings('ghostscript',gs_cmd);
end
handles(end+1)=uicontrol('style','edit', ...
    'position',[HOffset VOffset TabWidth-Margin-30 20], ...
    'string',gs_cmd, ...
    'horizontalalignment','left', ...
    'backgroundcolor',Active, ...
    'tag','ghostscript', ...
    'callback','d3d_qp ghostscript', ...
    'enable','on', ...
    'parent',mfig);
handles(end+1)=uicontrol('Style','pushbutton', ...
    'position',[HOffset+TabWidth-Margin-20 VOffset 20 20], ...
    'string','...', ...
    'tooltip','Browse for process definition file', ...
    'horizontalalignment','left', ...
    'tag','ghostscript_browse', ...
    'callback','d3d_qp ghostscript_browse', ...
    'enable','on', ...
    'parent',mfig);
...
handles(end+1)=uicontrol('style','text', ...
    ...
    'parent',mfig);
%-------------
if matlabversionnumber >= 7 && matlabversionnumber < 9
    VOffset = VOffset-20;
    handles(end+1)=uicontrol('style','checkbox', ...
...
function gs_cmd = locate_ghostscript
% first search in PATH
Path = getenv('PATH');
Folders = strsplit(Path,pathsep);
for i = 1:length(Folders)
    fp = Folders{i};
    gs_cmd = [fp,filesep,'gswin64c.exe'];
    if exist(gs_cmd,'file')
        return
    end
end
% if not found, search specific folders
Folders = {'C:\Program Files\gs\gs10.00.0\bin'};
for i = 1:length(Folders)
    fp = Folders{i};
    gs_cmd = [fp,filesep,'gswin64c.exe'];
    if exist(gs_cmd,'file')
