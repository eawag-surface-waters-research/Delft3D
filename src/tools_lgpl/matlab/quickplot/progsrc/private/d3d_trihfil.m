...
if k==1
   ...
   DataProps(id,:)=[];
   id=strmatch('vertical velocity',DataProps(:,1),'exact');
   DataProps(id,:)=[];
   id=strmatch('discharge',DataProps(:,1),'exact');
   DataProps(id,:)=[];
end
...
Info=vs_disp(FI,'his-series','ZCURU');
if isfield(Info,'SizeDim') && length(Info.SizeDim)>1 && Info.SizeDim(2)==1
   id=strmatch('horizontal velocity',DataProps(:,1),'exact');
   DataProps(id,:)=[];
   id=strmatch('discharge',DataProps(:,1),'exact');
   DataProps(id,:)=[];
