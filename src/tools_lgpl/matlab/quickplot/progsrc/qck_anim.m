...
switch cmd
         ...
         while doloop
            ...
            for i0=1:NSteps,
               ...
               for iobj=1:length(UDh)
                  ...
                  if t_==0
                     AnimObj.PlotState.SubField{1} = i;
                  else
                     AnimObj.PlotState.Selected{t_} = i;
                  end
      ...
      try
         ...
         for i=1:length(sld)
            if ishandle(sld(i))
               ...
               Str=sprintf('%i',t);
               if t_==0
                  [Chk,sflds] = qp_getdata(AnimObj.PlotState.FI, ...
                     AnimObj.PlotState.Domain, ...
                     AnimObj.PlotState.Props,'subfields',t);
                  if Chk
                     Str=sflds{1};
                  end
               end
               set(sld(i),'value',t,'tooltip',[DimStr{t_+1},Str]);
            end
         end
         ish=ishandle(sld);
         set(sld(ish),'vis','on')
   ...
   case 'slider'
      ...
      set(sld,'tooltip',[DimStr{t_+1},Str]);
   case 'animselect'
      ...
      set(animslid,'tooltip',[DimStr{AS(1).Fld(1)+1},Str]);
      if NoUpdateNec
         return
      end
...
for iobj=1:length(UDh)
   ...
   %
   if t_==0
      AnimObj.PlotState.SubField{1} = t;
   else
      AnimObj.PlotState.Selected{t_} = t;
   end
