 ...
 *   $Header: /upc/share/CVS/netcdf-3/ncgen/main.c,v 1.10 2005/11/16 18:04:48 ed Exp $
 *********************************************************************/
	...
	char *argv[])
{
	...
	case 'x':		/* set nofill mode to speed up creation of large files */
	  nofill_flag = 1;
	  break;
        case 'v':		/* for creating 64-bit offset files, specify version 2 */
	    {
		...
		/* The 64-bit offset version (2)  should only be used if actually needed */
...
#ifdef USE_NETCDF4
		/* NetCDF-4 */
		else if (strcmp(version_name, "3") == 0 || 
		    strcmp(version_name, "netcdf-4") == 0) {
		    cmode_modifier |= NC_NETCDF4;
		}
		/* NetCDF-4 classic */
		else if (strcmp(version_name, "4") == 0 ||
		    strcmp(version_name, "netcdf-4-classic") == 0) {
		...
		{
		   derror("Invalid format, try 1, 2, 3, or 4");
		   return 2;
		}
	    }
	  break;
