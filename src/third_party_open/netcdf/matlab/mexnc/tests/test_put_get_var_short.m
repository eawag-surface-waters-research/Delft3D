...
input_data = [1:1:prod(dimlen)];
input_data = reshape ( input_data, dimlen(1), dimlen(2) );
input_data = int16 ( input_data(2:4,1:3) );

	input_data = input_data';
	start = [1 0]; count = [3 3];

status = mexnc ( 'PUT_VARA_SHORT', ncid, varid, start,count,input_data );
if ( status < 0 )
...
[output_data, status] = mexnc ( 'GET_VARA_SHORT', ncid, varid, start,count);
if ( status < 0 )
...
input_data = [1:1:prod(dimlen)];
input_data = reshape ( input_data, dimlen(1), dimlen(2) );
input_data = int16 ( input_data(1:2:end,1:2:end) );
[r,c] = size(input_data);

	input_data = input_data';
start = [0 0]; count = [r c]; stride = [2 2];

status = mexnc ( 'PUT_VARS_SHORT', ncid, varid, start,count,stride,input_data );
if ( status < 0 )
...
[output_data, status] = mexnc ( 'GET_VARS_SHORT', ncid, varid, start,count,stride );
if ( status < 0 )
