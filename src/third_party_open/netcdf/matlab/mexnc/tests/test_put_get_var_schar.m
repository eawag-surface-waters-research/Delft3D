function test_put_get_var_schar ( ncfile )
...
%     [PUT,GET]_VAR_SCHAR: Write a 6x4 array of monotonically increasing values, read them back.
%     [PUT,GET]_VAR1_SCHAR:  write and read 255 to disk
%     [PUT,GET]_VARA_SCHAR:  write and read a 3x3 chunk
%     [PUT,GET]_VARS_SCHAR:  write and read the 6x4 array, subset by factor of 2, stride of 2
%     [PUT,GET]_VARM_SCHAR:  normally we want to transpose the data before writing with
...
input_data = int8( input_data );

input_data = input_data';

status = mexnc ( 'PUT_VAR_SCHAR', ncid, varid, input_data);
if ( status ~= 0 )
	ncerr_msg = mexnc ( 'strerror', status );
	msg = sprintf ( '%s:  PUT_VAR_SCHAR failed, (%s)\n', mfilename, ncerr_msg );
	error ( msg );
end


[output_data, status] = mexnc ( 'GET_VAR_SCHAR', ncid, varid );
if ( status ~= 0 )
	msg = sprintf ( '%s:  GET_VAR_SCHAR failed, msg ''%s''\n', mfilename, mexnc ( 'strerror', status ) );
	error ( msg );
end
...
if (any(d))
	msg = sprintf ( '%s:  values written by PUT_VAR_SCHAR do not match what was retrieved by GET_VAR_SCHAR\n', mfilename  );
	error ( msg );
end

fprintf ( 1, 'PUT_VAR_SCHAR succeeded\n' );
fprintf ( 1, 'GET_VAR_SCHAR succeeded\n' );
...
input_data = 255;
input_data = int8 ( input_data );
status = mexnc ( 'PUT_VAR1_SCHAR', ncid, varid, [2 2], input_data );
if ( status < 0 )
	msg = sprintf ( '%s:  PUT_VAR1_SCHAR failed\n', mfilename );
	error ( msg );
end
...
[output_data, status] = mexnc ( 'GET_VAR1_SCHAR', ncid, varid, [2 2] );
if ( status < 0 )
	msg = sprintf ( '%s:  GET_VAR1_SCHAR failed, msg ''%s''\n', mfilename, mexnc ( 'strerror', status ) );
	error ( msg );
end

d = max(abs(double(output_data)-double(input_data)));
if (d > 0)
	msg = sprintf ( '%s:  values written by PUT_VAR1_SCHAR do not match what was retrieved by GET_VAR_SCHAR\n', mfilename  );
	error ( msg );
end

fprintf ( 1, 'PUT_VAR1_SCHAR succeeded\n' );
fprintf ( 1, 'GET_VAR1_SCHAR succeeded\n' );
...
input_data = [1:1:prod(dimlen)];
input_data = reshape ( input_data, dimlen(1), dimlen(2) );
input_data = int8( input_data(2:4,1:3) );

	input_data = input_data';
	start = [1 0]; count = [3 3];

status = mexnc ( 'PUT_VARA_SCHAR', ncid, varid, start, count, input_data );
if ( status < 0 )
	msg = sprintf ( '%s:  PUT_VARA_SCHAR failed\n', mfilename );
	error ( msg );
end
...
[output_data, status] = mexnc ( 'GET_VARA_SCHAR', ncid, varid, start, count );
if ( status < 0 )
	msg = sprintf ( '%s:  GET_VARA_SCHAR failed, msg ''%s''\n', mfilename, mexnc ( 'strerror', status ) );
	error ( msg );
end
...
d = max(abs(double(output_data)-double(input_data)))';
if (any(d))
	msg = sprintf ( '%s:  values written by PUT_VARA_SCHAR do not match what was retrieved by GET_VARA_SCHAR\n', mfilename  );
	error ( msg );
end

fprintf ( 1, 'PUT_VARA_SCHAR succeeded\n' );
fprintf ( 1, 'GET_VARA_SCHAR succeeded\n' );
...
input_data = [1:1:prod(dimlen)];
input_data = reshape ( input_data, dimlen(1), dimlen(2) );
input_data = int8( input_data(1:2:end,1:2:end));
[r,c] = size(input_data);

input_data = input_data';
start = [0 0]; count = [r c]; stride = [2 2];

status = mexnc ( 'PUT_VARS_SCHAR', ncid, varid, start,count,stride, input_data );
if ( status < 0 )
	msg = sprintf ( '%s:  PUT_VARS_SCHAR failed\n', mfilename );
	error ( msg );
end
...
[output_data, status] = mexnc ( 'GET_VARS_SCHAR', ncid, varid, start,count,stride);
if ( status < 0 )
	msg = sprintf ( '%s:  GET_VARS_SCHAR failed, msg ''%s''\n', mfilename, mexnc ( 'strerror', status ) );
	error ( msg );
end
...
d = max(abs(double(output_data)-double(input_data)))';
if (any(d))
	msg = sprintf ( '%s:  values written by PUT_VARS_SCHAR do not match what was retrieved by GET_VARS_SCHAR\n', mfilename  );
	error ( msg );
end

fprintf ( 1, 'PUT_VARS_SCHAR succeeded\n' );
fprintf ( 1, 'GET_VARS_SCHAR succeeded\n' );
...
fprintf ( 1, 'GET_VAR_SCHAR succeeded on a singleton\n' );
fprintf ( 1, 'PUT_VAR_SCHAR succeeded on a singleton\n' );
...
input_data = int8(3.14);
status = mexnc ( 'PUT_VAR1_SCHAR', ncid, varid, 0, input_data );
if status
	error ( mexnc('strerror',status) );
end

[output_data, status] = mexnc ( 'GET_VAR1_SCHAR', ncid, varid, 0 );
if status
	error ( mexnc('strerror',status) );
end
...
d = max(abs(double(output_data)-double(input_data)))';
if (any(d))
	msg = sprintf ( '%s:  values written by PUT_VAR1_SCHAR do not match what was retrieved by GET_VAR1_SCHAR\n', mfilename  );
	error ( msg );
end
...
fprintf ( 1, 'GET_VAR1_SCHAR succeeded on a singleton\n' );
fprintf ( 1, 'PUT_VAR1_SCHAR succeeded on a singleton\n' );
...
d = max(abs(double(output_data)-double(input_data)))';
if (any(d))
	msg = sprintf ( '%s:  values written by PUT_VARM_SCHAR do not match what was retrieved by GET_VARM_SCHAR\n', mfilename  );
	error ( msg );
end



fprintf ( 1, 'PUT_VARM_SCHAR succeeded\n' );
fprintf ( 1, 'GET_VARM_SCHAR succeeded\n' );
