...
C
              ...
              RWRK(IPSN+1)=YINT
C
              IF (.NOT.(IWRK(IPE1+5).NE.IWRK(IPE1+4))) GO TO 10171
...
C
              ...
              RWRK(IPSN+1)=YINT
C
              IF (.NOT.(IWRK(IPE2+5).NE.IWRK(IPE2+4))) GO TO 10177
...
C
            ...
            RWRK(IPSN+1)=YINT
C
            IF (.NOT.(IWRK(IPE1+5).NE.IWRK(IPE1+4))) GO TO 10212
...
C
            IF (.NOT.(IPP1.NE.IPP2)) GO TO 10214
C
...
C
              IF (.NOT.(IPPL.EQ.IPP1)) GO TO 10215
                IPPL=IWRK(IPP1+2)
              GO TO 10216
10215         CONTINUE
                ISPL=IPPL
10217           CONTINUE
                  IF (.NOT.(IWRK(ISPL+2).EQ.IPP1)) GO TO 10218
...
C edge.
C
        IF (.NOT.(IWRK(ITMP+4).EQ.0)) GO TO 10223
          IF (.NOT.(INNP.LT.1)) GO TO 10224
            INNP=INNP+LCCP
          GO TO 10225
10224     CONTINUE
          IF (.NOT.(INNP.GT.LCCP)) GO TO 10226
            INNP=INNP-LCCP
10225     CONTINUE
10226     CONTINUE
          XCNP=XCCP(INNP)
        ...
        GO TO 10227
10223   CONTINUE
          IF (.NOT.(INNP.LT.1)) GO TO 10228
            INNP=INNP+LCSP
          GO TO 10229
10228     CONTINUE
          IF (.NOT.(INNP.GT.LCSP)) GO TO 10230
            INNP=INNP-LCSP
10229     CONTINUE
10230     CONTINUE
          XCNP=XCSP(INNP)
...
C
        IF (.NOT.(YCNP.GE.YTOS)) GO TO 10231
C
...
C
          IF (.NOT.(IWRK(ITMP+9).NE.0)) GO TO 10232
C
C The edge is contributing to a polygon.  Form a subsidiary polygon
C node to add to that polygon.
C
            IF (.NOT.(IG03.NE.0)) GO TO 10233
              IPSN=IG03
              IG03=IWRK(IG03)
            GO TO 10234
10233       CONTINUE
              IPWU=IPWU-3
              IF (.NOT.(IPWU.LE.IPWL)) GO TO 10235
                GO TO 10017
10235         CONTINUE
              IPSN=IPWU
10234       CONTINUE
...
C is appropriate.
C
            IF (.NOT.(IWRK(ITMP+5).NE.IWRK(ITMP+4))) GO TO 10237
...
C
          ...
          RWRK(ITMP+2)=YCNP
C
          IF (.NOT.(YCNP.NE.YTOS)) GO TO 10239
...
C
          IF (.NOT.(IWRK(ITMP+6).EQ.0)) GO TO 10242
            IERR=7
            GO TO 10161
10242     CONTINUE
C
...
C
          IF (.NOT.(IPP1.NE.0.OR.IPP2.NE.0)) GO TO 10244
C
            IF (.NOT.(IPP1.EQ.0.OR.IPP2.EQ.0)) GO TO 10245
              IERR=8
              GO TO 10161
10245       CONTINUE
C
            IF (.NOT.(IG03.NE.0)) GO TO 10247
              IPSN=IG03
              IG03=IWRK(IG03)
            GO TO 10248
10247       CONTINUE
              IPWU=IPWU-3
              IF (.NOT.(IPWU.LE.IPWL)) GO TO 10249
                GO TO 10017
10249         CONTINUE
              IPSN=IPWU
10248       CONTINUE
C
            ...
            RWRK(IPSN+1)=YTOS
C
            IF (.NOT.(IWRK(ITMP+5).NE.IWRK(ITMP+4))) GO TO 10251
              IWRK(IPSN+2)=IWRK(IPP1)
              IWRK(IPP1)=IPSN
            GO TO 10252
10251       CONTINUE
...
C See if the meeting edges are contributing to the same polygon.
C
            IF (.NOT.(IPP1.NE.IPP2)) GO TO 10253
C
C They aren't.  Append the subsidiary nodes of one polygon to the other.
C
              IF (.NOT.(IWRK(ITMP+5).NE.IWRK(ITMP+4))) GO TO 10254
...
C garbage list for 3-word nodes, so that it can be re-used.
C
              IF (.NOT.(IPPL.EQ.IPP1)) GO TO 10256
                IPPL=IWRK(IPP1+2)
              GO TO 10257
10256         CONTINUE
                ISPL=IPPL
10258           CONTINUE
                  IF (.NOT.(IWRK(ISPL+2).EQ.IPP1)) GO TO 10259
                    IWRK(ISPL+2)=IWRK(IPP1+2)
                    GO TO 10260
10259             CONTINUE
                  ISPL=IWRK(ISPL+2)
