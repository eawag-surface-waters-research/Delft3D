...
#     File "TMP_config_flow2d3d.xml" is created and removed after the calculation
# --------------------------------------------------------------------
...
proc getInput {channel} {
    ...
    if { [gets $channel line] >= 0 } {
        ...
        if { [catch {close $channel} errmsg] } {
            puts "\nERROR: $errmsg"
            puts "       - Is \"vcredist_x86 (2008 SP1).exe\" installed?"
...
proc usage { } {
   ...
   puts "                       Name    = flow2d3d"
   puts "                       MdfFile = f34"
   puts "    -keepXML     : Do not remove output file \"$::xmlFile\""
}
...
# MAIN
#
set xmlFile "TMP_config_flow2d3d.xml"
set debug 0
...
if {$debug} {
   ...
   # puts "location1: [file dirname [file normalize [info script deltares_hydro.tcl]]]"
   # puts "location2: [file dirname [file normalize [info nameofexecutable]]]"
   # puts "location3: [file normalize [file join [pwd] $argv0]]"
   # puts "location4: [file dirname [info script]]"
}
