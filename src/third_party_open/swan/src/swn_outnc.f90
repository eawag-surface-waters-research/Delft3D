...
!
    implicit none
    private
    public :: swn_outnc_openblockfile, swn_outnc_close_on_end, swn_outnc_colspc, swn_outnc_spec, &
              swn_outnc_appendblock, record_index, stnames, stnames_init, ncoffset
!
!   Module parameters
!
...
!
!  1. Updates
!     41.70 Added support for multiple compute commands and multiple specout requests
!
!  2. Purpose
!
...
!
!  1. Updates
!     41.70 Do not close binary files. Breaks on multiple compute commands
!
!  2. Purpose
!
...
!PUN      iproc = MYPROC
      ...
      allocate(lspcaux%hs(MIP), lspcaux%depth(MIP), lspcaux%ux(MIP), lspcaux%uy(MIP), &
               ...
               lspcaux%xp(MIP), lspcaux%yp(MIP), ips(MIP))
!
! a. decide wether relative and 2D spectra are required
!
      irq      = OQI(2)
...
!
      if ( recordaxe(irq)%nstatm ) then
          ...
          call record_index(ncid, recordaxe(irq), xpctime, ri, .false., '')
      else
          ...
          call record_index(ncid, recordaxe(irq), pnr, ri, .false., '' )
      end if
...
!
    ...
    subroutine swn_outnc_appendblock(myk, mxk, ivtype, nref, irq, data, excv, col)
        ...
        if (myk == 1) then
            ...
            name = STNAMES(ivtype,col)
            if (name == ' ' .and. ivtype == 1) name = 'x'
            if (name == ' ' .and. ivtype == 2) name = 'y'
            call agnc_add_mapdata(ncid, name, ri, data, excv, skip_range_error)
        end if
    ...
    subroutine swn_outnc_close_on_end(nref, irq)
        ...
        ncid = nref
        if ( present(irq) ) ncid = nrefcp(irq) + ncoffset(irq)
    ...
    subroutine record_index(ncid, recordaxe, xpctime, ri, reading, filename)
        ...
        character(len=*),                     intent(   in) :: filename ! for use in error message
        ...
        if ( ri < 0 ) then
            if (reading) then
                call MSGERR(2, "first time not available in input netcdf " // filename)
                ri = 1
            else
            ...
            if (reading) then
                call msgerr(2, "last time not available in input netcdf " // filename)
            else
            ...
            if (myk == 1) then
                ...
                if ( abs(ALPC) < epsilon(1.) .and. OPTG == 1 ) then
                    ...
                    if (present(xgrdgl) .and. present(ygrdgl)) then
                       ...
                       mapgrid%longitude(:,1) = -999.0
                       mapgrid%latitude(1,:)  = -999.0
                    end if
                    mapgrid%mdc  = .false.
                else
                    ...
                    if (present(xgrdgl) .and. present(ygrdgl)) then
                       ...
                       mapgrid%longitude = -999.0
                       mapgrid%latitude  = -999.0
                    end if
                    mapgrid%mdc  = .true.
                end if
                ! curvilinear grids may contain the EXCEPTION value of the bottom
