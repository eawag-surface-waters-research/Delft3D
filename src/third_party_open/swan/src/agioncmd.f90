...
!
      module agioncmd
        ...
        type, public :: mapgrid_type
            ...
            real                                     :: fillValue
            logical                                  :: isCurvi = .false.
            logical                                  :: flippedY = .false.
        ...
        type, public :: recordaxe_type
            integer(kind=8), dimension(:), allocatable :: content
            integer                                    :: delta
        ...
        public :: datevec
        public :: agnc_add_spcdata_density, agnc_collect_spcmeta
        public :: agnc_get_mapgrid, agnc_get_pntgrid
        public :: agnc_set_mapgrid, agnc_set_pntgrid
        public :: agnc_define_mapvariable, agnc_define_pntvariable
        public :: agnc_get_varid_by_name
...
!
        ...
        ! the dimension variables in the file.
            ...
            if ( present(recordaxe)) then
                ...
                if ( axe_is_regular(recordaxe_tmp%content, recordaxe_tmp%ncontent, .true.) ) then
                    recordaxe = recordaxe_tmp
                else
                    if ( axe_is_regular(recordaxe%content, recordaxe%ncontent, .true.)) then
...
!       agnc_set_spcgrid (ncid, spcgrid)                  done
        ...
        ! is predeclared for the month of trecordaxe%content(1)
            ...
            integer                                               :: t1(6), t2(6), i
            integer                                               :: ntime1    ! number timesteps already on NetCDF-file
            integer                                               :: ntime2    ! number of timesteps in recordaxe
            integer                                               :: length(1) ! number of timesteps to be appended
            integer                                               :: start(1)  ! start vector for nf90_put_var
            ...
            if (.not. axe_is_regular(recordaxe%content, recordaxe%ncontent) ) then
                call close_ncfile(ncid)
                call agnc_set_error( 'Failed to append data to this file. It would result in a irregular record axis');
            end if
            !
            ! update times on NetCDF file; only append times not already on file
            !
            call nccheck ( nf90_inquire_dimension(ncid, recordaxe%varid, len = ntime1) )
            ntime2 = recordaxe%ncontent

            if (ntime2 > ntime1) then
                start(1)  = ntime1 + 1
                length(1) = ntime2 - ntime1
                call nccheck ( nf90_put_var(ncid, recordaxe%varid, recordaxe%content(ntime1+1:ntime2), start, length) )
            endif
        ...
        subroutine agnc_set_mapgrid (ncid, mapgrid)
            ...
            call put_mapgrid_chunked_write(ncid, mapgrid%lon_varid, mapgrid%longitude)
            if ( mapgrid%mdc ) then
                call put_mapgrid_chunked_write(ncid, mapgrid%lat_varid, mapgrid%latitude)
            else
...
!
!       actual write of coordinates
!       write in chunks of 256*256 to avoid problems in NetCDF writer
!       also replace dummy of -1e10 by NF90_FILL_FLOAT
        subroutine put_mapgrid_chunked_write(ncid, varid, values)
            ...
            nx = size(values,1)
            ny = size(values,2)
            chunksizex = min(chunksize, nx)
            chunksizey = min(chunksize, ny)

            allocate(vloc(chunksizex, chunksizey))

            do sx = 1, nx, chunksizex
                cx = minval( (/chunksizex, nx-sx+1/) )
                do sy = 1, ny, chunksizey
                    cy = minval( (/chunksizey, ny-sy+1/) )

                    if (cx /= chunksizex .or. cy /= chunksizey) then
                        vloc = 0.
                    endif
                    do lxi=1,cx
                        do lyi=1,cy
                            tmp = values((sx + lxi - 1) , (sy + lyi - 1) )
                            if (tmp == -1e10) then
                                vloc(lxi,lyi) = NF90_FILL_FLOAT
                            else
                                vloc(lxi,lyi) = tmp
                            endif
                        end do
                    ...
                    nf90_stat = nf90_put_var(ncid, varid, vloc(1:cx,1:cy), (/sx,sy/), (/cx, cy/))
                    call nccheck(nf90_stat)
                enddo
            enddo
            deallocate(vloc)
        end subroutine put_mapgrid_chunked_write
...
!       agnc_get_recordaxe
        subroutine agnc_get_mapgrid(ncid, mapgrid)
            ...
            integer                             :: ndims, mp, np
            real                                :: xlen, ylen, fill_value
            real, dimension(:), allocatable     :: lattmp
            real, dimension(:,:,:), allocatable :: bndtmp
            ...
            if ( ndims == 1 ) then
                ...
                call nccheck ( nf90_get_att(ncid, mapgrid%lon_varid, "_FillValue", fill_value), "_FillValue" )
                mapgrid%fillValue = fill_value
                ...
                if (ndims == 2) then
                    ...
                    call nccheck ( nf90_get_var(ncid, mapgrid%lon_varid, mapgrid%longitude) )
                    if (cornersAreFilled(mapgrid, fill_value)) then
                        xlen = mapgrid%longitude(mapgrid%nx,1) - mapgrid%longitude(1,1)
                        ylen = mapgrid%latitude(mapgrid%nx,1)  - mapgrid%latitude(1,1)

                        mapgrid%alpc = atan2(xlen, ylen)
                        ! greatcircle??
                        mapgrid%dx   = hypot( xlen, ylen ) / ( mapgrid%nx - 1 )

                        xlen = mapgrid%longitude(1,mapgrid%ny) - mapgrid%longitude(1,1)
                        ylen = mapgrid%latitude(1,mapgrid%ny)  - mapgrid%latitude(1,1)
                        mapgrid%dy = hypot ( xlen, ylen) / ( mapgrid%ny - 1 )
                    else
                        call searchActivePoint(mapgrid, mp, np, fill_value)
                        call fillMapgridProps(mapgrid, mp, np)
                    endif
                else if (mapgrid%hasBounds) then
                    allocate(bndtmp(4, mapgrid%nx, mapgrid%ny))
                    call nccheck ( nf90_get_var(ncid, mapgrid%lat_varid, bndtmp) )
                    call getMeanOfBounds(mapgrid, mapgrid%latitude, bndtmp)
                    call nccheck ( nf90_get_var(ncid, mapgrid%lon_varid, bndtmp) )
                    call getMeanOfBounds(mapgrid, mapgrid%longitude, bndtmp)
                    deallocate(bndtmp)

                    call searchActivePoint(mapgrid, mp, np, fill_value)
                    call fillMapgridProps(mapgrid, mp, np)
                else
                    call agnc_set_error("wrong number of dimensions in agnc_get_mapgrid")
                endif
        ...
        subroutine getMeanOfBounds(mapgrid, coordsarray, bnd)
            ...
            do m = 1, mapgrid%nx
                do n = 1, mapgrid%ny
                    coordsarray(m,n) = (bnd(1,m,n) + bnd(2,m,n) + bnd(3,m,n) + bnd(4,m,n)) / 4.0
                enddo
            enddo
        end subroutine getMeanOfBounds
        ...
        subroutine searchActivePoint(mapgrid, mp, np, fill_value)
            ...
            logical :: ok
            integer :: i, j, k
            real, parameter :: ratioM(5) = [0.5, 0.25, 0.75, 0.75, 0.25]
            real, parameter :: ratioN(5) = [0.5, 0.25, 0.25, 0.75, 0.75]

            do k = 1, size(ratioM)
                mp = ratioM(k) * mapgrid%nx
                np = ratioN(k) * mapgrid%ny
                ok = .true.
                search: &
                do i = -1,1
                   do j = -1,1
                       if (mapgrid%longitude(mp+i,np+j) == fill_value) then
                           ok = .false.
                           exit search
                       endif
                   enddo
                ...
                if (ok) exit
            enddo
            if (.not. ok) call agnc_set_error("no active point found")
        end subroutine searchActivePoint
        ...
        function cornersAreFilled(mapgrid, fill_value) result(chk)
            ...
            do k = 1, 2
                if (k == 1) then
                    coords => mapgrid%longitude
                else
                    coords => mapgrid%latitude
                endif
                do n = 1, mapgrid%nx, mapgrid%nx-1
                    do m = 1, mapgrid%ny, mapgrid%ny-1
                        if (coords(n,m) == fill_value) then
                            chk = .false.
                            exit outer
                        endif
                    enddo
        ...
        end function cornersAreFilled

        subroutine fillMapgridProps(mapgrid, mp, np)
            use m_genarr, only : xcgrid, ycgrid
            use swcomm2,   only: xoffs, yoffs
            type (mapgrid_type), intent (inout)   :: mapgrid
            integer,             intent (in)      :: mp, np

            real :: xlen, ylen, diff1x, diff2x, diff1y, diff2y
            integer :: j

            xlen = mapgrid%longitude(mp+1,np) - mapgrid%longitude(mp,np)
            ylen = mapgrid%latitude(mp+1,np)  - mapgrid%latitude(mp,np)

            mapgrid%alpc = atan2(xlen, ylen)
            ! greatcircle??
            mapgrid%dx   = hypot( xlen, ylen )

            xlen = mapgrid%longitude(mp,np+1) - mapgrid%longitude(mp,np)
            ylen = mapgrid%latitude(mp,np+1)  - mapgrid%latitude(mp,np)
            mapgrid%dy = hypot ( xlen, ylen)
            mapgrid%isCurvi = .true.

            ! check on flipped y-axis
            diff1x = 0.0;  diff2x = 0.0; diff1y = 0.0; diff2y = 0.0
            do j = 1, mapgrid%ny
                if (mapgrid%longitude(mp,j) /= mapgrid%fillValue) then
                    diff1x = diff1x + abs(xcgrid(mp,mapgrid%ny + 1 - j) + xoffs - mapgrid%longitude(mp,j))
                    diff2x = diff2x + abs(xcgrid(mp,j) + xoffs- mapgrid%longitude(mp,j))
                endif
                if (mapgrid%latitude(mp,j) /= mapgrid%fillValue) then
                    diff1y = diff1y + abs(ycgrid(mp,mapgrid%ny + 1 - j) + yoffs - mapgrid%latitude(mp,j))
                    diff2y = diff2y + abs(ycgrid(mp,j) + yoffs- mapgrid%latitude(mp,j))
                endif
            enddo
            mapgrid%flippedY = diff1x + diff1y < diff2x + diff2y
        end subroutine fillMapgridProps
        ...
        subroutine agnc_get_pntgrid(ncid, pntgrid)
            ...
            ierr = nf90_get_att(ncid, pntgrid%lon_varid, 'units', units)
            call convertCstring(units)
            if (ierr == nf90_noerr) then
                if (eqcstr('meter', trim(units))) then
                    pntgrid%lunit = units
                endif
            endif
        ...
        subroutine agnc_get_spcgrid(ncid, spcgrid)
            ...
            if ( spcgrid%frq_varid > 0 ) then
                ...
                if ( spcgrid%dir_varid > 0 ) then
                    ...
                    ierr = nf90_get_att(ncid, spcgrid%dir_varid, "units", unit)
                    if (ierr == nf90_noerr) then
        ...
        subroutine agnc_get_recordaxe(ncid, recordaxe)
            ...
            integer                                    :: factor, dvec(6), unitsLength
            logical                                    :: is_regular
            ...
            call nccheck ( nf90_inquire_attribute(ncid, recordaxe%varid, 'units', len = unitsLength) )
            call nccheck ( nf90_get_att(ncid, recordaxe%varid, 'units', units), 'units' )
            call nccheck ( nf90_inquire_variable(ncid, recordaxe%varid, raname) )
            allocate(recordaxe%content(recordaxe%ncontent))

            if (raname /= 'run') then

                call agnc_scan_time(units(1:unitsLength), factor, dvec)

                allocate(dvalues(recordaxe%ncontent))
                call nccheck ( nf90_get_var(ncid, recordaxe%varid, dvalues, (/1/), (/recordaxe%ncontent/) ) )
            ...
            if (recordaxe%ncontent > 1) then
                ...
                is_regular = axe_is_regular(recordaxe%content, recordaxe%ncontent)
                if (is_regular) then
                    recordaxe%first = 1
                    recordaxe%last = recordaxe%nContent
                else
                    is_regular = axe_regular_range(recordaxe)
                endif
        ...
        subroutine agnc_get_recordaxe_ids(ncid, dimid, varid, ncontent, raname)
            ...
            ! Note that the assumption is that the record axe dimension has a corresponding variable

            ierr = nf90_inquire_dimension_list(ncid, dimid, dnames)
            dim_found = (dimid > 0)
            ...
            if ( present(varid) ) ierr = nf90_inquire_variable_list(ncid, varid, [dname])
            if ( present(raname)) raname = dname

        end subroutine agnc_get_recordaxe_ids

        subroutine find_projection_coord(ncid, dimid, varid, stdname)
            !
            ! description:
            ! helper function to find identifier for coordinates based on standard_name
            ! successful if lon_dimid > 0
            !
            integer             , intent( in)                   :: ncid        !< NetCDF file identifier
            integer             , intent(out)                   :: dimid       !< dimension identifier
            integer             , intent(out)                   :: varid       !< dimension identifier
            character(len=*)    , intent(in)                    :: stdname     !< standard_name for dimension

            integer            :: nvar, nDims, ierr, i, varI
            character(len=40)  :: varstdname, nameDim(1)

            logical, external :: eqcstr

            dimid = 0
            ierr = nf90_inquire(ncid, nDims, nvar)
            ...
            if (ierr == 0) then
                do i = 1, nDims
                    ...
                    if (eqcstr(trim(varstdname), trim(stdname))) then
                        dimid = i
                        varId = varI
                        exit
                    endif
        ...
        end subroutine find_projection_coord

        subroutine agnc_get_ll_dimids(ncid, mapgrid)
            integer             , intent( in)                   :: ncid
            ...
            logical :: ready
            character(len=*), parameter :: namesx(2) = ["projection_x_coordinate", "longitude              "]
            character(len=*), parameter :: namesy(2) = ["projection_y_coordinate", "latitude               "]

            ready = .false.
            ierr1 = 0; ierr2 = 0
            do i = 1, size(namesx)
                call find_projection_coord(ncid, mapgrid%lon_dimid, mapgrid%lon_varid, namesx(i))
                call find_projection_coord(ncid, mapgrid%lat_dimid, mapgrid%lat_varid, namesy(i))
                if ( mapgrid%lon_dimid > 0 .and. mapgrid%lat_dimid > 0) then
                    ready = .true.
                    exit
                endif
            enddo
            ...
            if (.not. ready) then
                ierr1 = nf90_inquire_dimension_list(ncid, mapgrid%lon_dimid, ["longitude", "x        ", "xc       ", "lon      "])
                ierr2 = nf90_inquire_dimension_list(ncid, mapgrid%lat_dimid, ["latitude ", "y        ", "yc       ", "lat      "])
                ready = ( mapgrid%lon_dimid > 0 .and. mapgrid%lat_dimid > 0)
            endif

            if (.not. ready) then
                ierr1 = nf90_inquire_variable_list(ncid, mapgrid%lon_varid, ["longitude", "x        ", "lon      "])
                ierr2 = nf90_inquire_variable_list(ncid, mapgrid%lat_varid, ["latitude ", "y        ", "lat      "])
                ready = ( mapgrid%lon_varid > 0 .and. mapgrid%lat_varid > 0)
            endif
        ...
        subroutine agnc_get_griddef_map(ncid, mapgrid)
            ...
            else if (mapgrid%lon_varid > 0 .and. mapgrid%lat_varid > 0) then
                ! support for files generated by Waqua/Getdata
                call nccheck ( nf90_inquire_variable(ncid, mapgrid%lon_varid, ndims=ndim1 ))
                allocate(dimIDs(ndim1))
                call nccheck ( nf90_inquire_variable(ncid, mapgrid%lon_varid, dimids = dimIDs) )
                do i = 1, ndim1
                    call nccheck ( nf90_inquire_dimension(ncid, dimIDs(i), name = name, len=length) )
                    if (eqcstr(trim(name), 'n') .or. eqcstr(trim(name), 'row')) then
                        mapgrid%ny = length
                    else if (eqcstr(trim(name), 'm') .or. eqcstr(trim(name), 'col')) then
                        mapgrid%nx = length
                    else if (eqcstr(trim(name), 'bounds')) then
                        mapgrid%hasBounds = .true.
                    endif
                enddo
                deallocate(dimIDs)
            else
                call agnc_set_error("fall through in agnc_get_griddef_map")
            endif

            if (mapgrid%lon_varid == 0 .and. mapgrid%lat_varid == 0) then
        ...
        subroutine agnc_get_griddef_pnt(ncid, pntgrid)
            ...
            character(len=*), parameter :: node_names(2) = ['points', 'node  ']
            integer :: i, ierr

            do i = 1, size(node_names)
                 ierr = nf90_inq_dimid(ncid, node_names(i), pntgrid%pnt_dimid)
            ...
            if ( nf90_inq_varid(ncid, 'ips', pntgrid%ips_varid) /= nf90_noerr ) then
                ...
                call nccheck ( nf90_get_att(ncid, pntgrid%lon_varid, 'dimlen', pntgrid%xdimlen), 'dimlen' )
        ...
        subroutine agnc_get_griddef_spc(ncid, spcgrid)
            ...
            ! spectral grid
            ierr = nf90_inquire_dimension_list(ncid, spcgrid%frq_dimid, names_afreq)
            call nccheck (ierr)

            call nccheck ( nf90_inquire_dimension(ncid, spcgrid%frq_dimid, len=spcgrid%nfreq) )

            ierr = nf90_inquire_variable_list(ncid, spcgrid%frq_varid, names_afreq)
            call nccheck (ierr)

            ! direction only available when frequency is set
            spcgrid%dir_dimid = -1
            ierr =  nf90_inquire_dimension_list(ncid, spcgrid%dir_dimid, names_ndir)
            if (spcgrid%dir_dimid > 0) then
                call nccheck ( nf90_inquire_dimension(ncid, spcgrid%dir_dimid, len=spcgrid%ndir) )
                call nccheck ( nf90_inquire_variable_list(ncid, spcgrid%dir_varid, names_ndir) )
            else
                call removeErrMsg()
                spcgrid%dir_dimid = -1
                spcgrid%dir_varid = -1
                ...
                do sy=1,mapgrid%ny,chunksize
                    ...
                    if ( nf90_stat /= NF90_NOERR ) then
                        if ( values_in_range_float(ncid, varid, values, &
                                fill_value, scale_factor, add_offset) ) then
       ...
       subroutine agnc_add_spcdata_density(ncid, ti, e2d, spc_as_map)
        ...
        subroutine agnc_add_spcdata_2d(ncid, ti, energy, theta, spr, spc_as_map)
            ...
            if ( msc /= size(energy,2) ) then
                call agnc_set_error('#points of this run and the netcdf file is different')
            end if
...
!       compute_1d_spectra(energy, e1, th1, sp1, undef) done
        ...
        !> check if content is regular at some range
        function axe_regular_range(recordaxe) result (isregular)
            type (recordaxe_type), intent(inout)      :: recordaxe
            logical                                   :: isregular

            integer :: i, dthalf, dt, n, nhalf, first, last

            n = recordaxe%nContent
            nhalf = n / 2
            dthalf = recordaxe%content(nhalf + 1) - recordaxe%content(nhalf)

            first = -1; last = -1
            do i = nhalf-1, 1, -1
                dt = recordaxe%content(i + 1) - recordaxe%content(i)
                if (dt == dthalf) then
                    first = i
                else
                    exit
            ...
            recordaxe%first = first
            recordaxe%last = last
            isregular = (last /= -1 .and. first /= -1 .and. last - first > 1)
            if (isregular) recordaxe%delta = dthalf

        end function axe_regular_range

        function axe_is_regular_integer64(x, nx, fill_check) result (isregular)
            integer(kind=8), intent(in)                :: x(:)
            integer,         intent(in)                :: nx
            logical,         intent(in), optional      :: fill_check
            logical                                    :: isregular
            integer(kind=8), dimension(:), allocatable :: dx2
        ...
        function values_in_range_float(ncid, varid, values, fill_value, scale_factor, add_offset) result (inrange)
            integer,                    intent(   in)         :: ncid, varid
        ...
        function values_in_range_double(ncid, varid, values, fill_value, scale_factor, add_offset) result (inrange)
            integer,                    intent(   in)         :: ncid, varid
        ...
        subroutine agnc_scan_time(ustr, factor, dvec)
            character(len=*),      intent( in) :: ustr
            integer,               intent(out) :: factor, dvec(6)

            integer                            :: j, slen, nspace, ierr
            ...
            j      = index(ustr, 'since')
            if ( j < 1 ) then
                call agnc_set_error('Keyword "since" not found in unit string: ' // ustr)
            end if
            ...
            select case (fstr)
                ...
                case('day', 'days', 'd')
                    factor = 86400
                case('year', 'years')
                    factor = 31556926  ! int32(365.242198781 * 86400)
                case('week', 'weeks')
                    factor = 606864    ! int32(365.242198781 * 86400 / 52)
                case('month', 'months')
                    factor = 2629744   ! int32(365.242198781 * 86400 / 12)
                case default
                    call agnc_set_error("Time unit " // fstr // " not supported")
            end select
            ...
            if (nspace > 0 .and. nspace < slen) then
                j  = j+nspace
                read(ustr(j:slen),'(I2,A,I2,A,I2)', iostat=ierr) dvec(4), dlim1, dvec(5), dlim2, dvec(6)
            else
                ! no time provided
                dvec(4:6) = 0
            end if
